...
set(INSIGHTMAP_TARGETS_ROOT_FOLDER "INSIGHTMAP_targets")
set(INSIGHTMAP_SRC_ROOT_FOLDER "INSIGHTMAP_sources")
...
macro(INSIGHTMAP_ADD_SOURCE_DIR SRC_DIR SRC_VAR)
    ...
    unset(ARG)
    unset(GROUP_NAME)
endmacro(INSIGHTMAP_ADD_SOURCE_DIR)

# Macro to add source files to INSIGHTMAP library.
macro(INSIGHTMAP_ADD_SOURCES)
    set(SOURCE_FILES "")
    ...
    endforeach()
    set(INSIGHTMAP_SOURCES ${INSIGHTMAP_SOURCES} ${SOURCE_FILES} PARENT_SCOPE)
endmacro(INSIGHTMAP_ADD_SOURCES)

# Macro to add CUDA source files to INSIGHTMAP library.
macro(INSIGHTMAP_ADD_CUDA_SOURCES)
    set(SOURCE_FILES "")
    foreach(SOURCE_FILE ${ARGN})
        if(SOURCE_FILE MATCHES "^/.*")
            # Absolute path.
    ...
    set(INSIGHTMAP_CUDA_SOURCES
        ${INSIGHTMAP_CUDA_SOURCES}
        ${SOURCE_FILES}
        PARENT_SCOPE)
endmacro(INSIGHTMAP_ADD_CUDA_SOURCES)
...
macro(INSIGHTMAP_ADD_LIBRARY TARGET_NAME)
    # ${ARGN} will store the list of source files passed to this function.
    add_library(${TARGET_NAME} ${ARGN})
    set_target_properties(${TARGET_NAME} PROPERTIES FOLDER
        ${INSIGHTMAP_TARGETS_ROOT_FOLDER}/${FOLDER_NAME})
    install(TARGETS ${TARGET_NAME} DESTINATION lib/INSIGHTMAP/)
endmacro(INSIGHTMAP_ADD_LIBRARY)
...
macro(INSIGHTMAP_ADD_CUDA_LIBRARY TARGET_NAME)
    # ${ARGN} will store the list of source files passed to this function.
    cuda_add_library(${TARGET_NAME} ${ARGN})
    set_target_properties(${TARGET_NAME} PROPERTIES FOLDER
        ${INSIGHTMAP_TARGETS_ROOT_FOLDER}/${FOLDER_NAME})
    install(TARGETS ${TARGET_NAME} DESTINATION lib/INSIGHTMAP/)
endmacro(INSIGHTMAP_ADD_CUDA_LIBRARY)

# Replacement for the normal add_executable() command. The syntax remains the
# same in that the first argument is the target name, and the following
# arguments are the source files to use when building the target.
macro(INSIGHTMAP_ADD_EXECUTABLE TARGET_NAME)
    # ${ARGN} will store the list of source files passed to this function.
    add_executable(${TARGET_NAME} ${ARGN})
    set_target_properties(${TARGET_NAME} PROPERTIES FOLDER
        ${INSIGHTMAP_TARGETS_ROOT_FOLDER}/${FOLDER_NAME})
    target_link_libraries(${TARGET_NAME} ${INSIGHTMAP_LIBRARIES})
    install(TARGETS ${TARGET_NAME} DESTINATION bin/)
endmacro(INSIGHTMAP_ADD_EXECUTABLE)

# Wrapper for test executables.
macro(INSIGHTMAP_ADD_TEST TARGET_NAME)
    if(TESTS_ENABLED)
        # ${ARGN} will store the list of source files passed to this function.
        add_executable(${TARGET_NAME} ${ARGN})
        set_target_properties(${TARGET_NAME} PROPERTIES FOLDER
...
endmacro(INSIGHTMAP_ADD_TEST)

# Wrapper for CUDA test executables.
macro(INSIGHTMAP_ADD_CUDA_TEST TARGET_NAME)
    if(TESTS_ENABLED)
        # ${ARGN} will store the list of source files passed to this function.
...
endmacro(INSIGHTMAP_ADD_CUDA_TEST)
