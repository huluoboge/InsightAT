#!/bin/bash
# Build an InsightAT AppImage for CUDA 11.8
...
export INSIGHTAT_BUILD_DIR="${INSIGHTAT_BUILD_DIR:-${REPO_ROOT}/build-cuda-11.8}"
...
export VERSION="${VERSION:-${INSIGHTAT_BASE_VERSION}-cuda11.8}"
export APPIMAGE_OUT_DIR="${APPIMAGE_OUT_DIR:-${REPO_ROOT}/build-appimage-cuda11.8}"
...
exec "$(dirname "${BASH_SOURCE[0]}")/compile_appimage.sh"
