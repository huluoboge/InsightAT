...
- **文件**: `src/ui/dialogs/ProjectInfoDialog.cpp`（历史路径）
- **变更**:
...
- **主实现**: `src/ui/dialogs/CoordinateSystemConfigDialog.*`（历史路径）
- **核心 Widget**: `src/ui/widgets/CoordinateSystemConfigWidget.*`（历史路径）
- **集成点**: `src/ui/dialogs/ProjectInfoDialog.cpp`（历史路径）
- **编译配置**: [src/ui/CMakeLists.txt](../../../src/ui/CMakeLists.txt)
- **主窗口**: `src/ui/MainWindow.cpp`（历史路径）
