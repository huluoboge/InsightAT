...
### 问题 3: 加载逻辑与系统架构不一致
**原状态**: 自定义的加载逻辑  
**修复后**: 遵循 `src/Common/Project.cpp`（历史路径）的标准方式
...
- 坐标系数据库加载: `src/Common/Project.cpp`（历史路径）
- 坐标系工具: `src/ui/widgets/SpatialReferenceTool.*`（历史路径）
- UI 小部件: `src/ui/widgets/CoordinateSystemWidget.*`（历史路径）
