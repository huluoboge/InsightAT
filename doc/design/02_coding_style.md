...
```
src/
├── algorithm/               # 核心算法（严禁依赖 Qt、不依赖 database）
│   ├── modules/
│   │   ├── camera/          # 相机内参最小结构 + 去畸变（Intrinsics，无 database）
│   │   ├── extraction/      # 特征提取
│   │   ├── retrieval/       # 图像检索
│   │   ├── matching/        # 特征匹配
...
| 函数 / 方法 | `snake_case`（含 UI 槽与信号） | `gpu_ransac_F()`, `on_project_changed()` |
