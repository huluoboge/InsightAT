...
RenderTileImageLayer::RenderTileImageLayer(QObject* parent)
    : RenderLayer(parent), m_valid(false), m_textureCount(0), m_textureNames(NULL) {
  m_tileImageLoader = new TileImageLoader(this);
  m_imageStream = nullptr;
  m_pyramid = new RenderGridTile(this);
  QObject::connect(m_tileImageLoader, SIGNAL(send_update_tiles(const PyramidData*)), this,
                   SLOT(receive_update_tiles(const PyramidData*)), Qt::DirectConnection);
  m_geoCoordinate = true;
  m_pyramid->set_max_buffer_pool_tile_size(m_poolMaxSize);
  m_drawFrame = false;
  m_drawName = false;
}
...
RenderTileImageLayer::~RenderTileImageLayer() {
  if (m_tileImageLoader->isRunning()) {
    m_tileImageLoader->set_exit_requested(true);
...
bool RenderTileImageLayer::load(const std::string& file) {
  VLOG(1) << "RenderTileImageLayer::load";
  if (m_tileImageLoader->isRunning()) {
    m_tileImageLoader->set_exit_requested(true);
    m_tileImageLoader->wait();
  }
  ...
  //����Ϊ�˼��ٴ�����ƫ�ƽ�ƾ���
  set_position(trans[0], trans[3], 0);
  // m_localOrigin.setX(trans[0]);
  // m_localOrigin.setY(trans[3]);
  trans[0] = 0;
  trans[3] = 0;
  m_pyramid->set_transform(trans);
  m_pyramid->build_pyramid_auto_deeps(info.Columns(), info.Rows(), m_tileSize * 2);
  m_tileImageLoader->set_image_stream(m_imageStream);
  m_tileImageLoader->set_pyramid_tile(m_pyramid);
  m_tileImageLoader->set_exit_requested(false);
  m_tileImageLoader->start();
  m_file = QString::fromLocal8Bit(file.c_str());
  QFileInfo fileInfo(m_file);
  m_name = fileInfo.fileName();
  VLOG(1) << "RenderTileImageLayer::load_end";
...
void RenderTileImageLayer::setData(const std::vector<Tile*>& vecTiles) {
  ...
  for (int i = 0; i < m_textureCount; ++i) {
    ...
    int rgbType = GL_RGB;
    if (vecTiles[i]->band_count == 3) {
      rgbType = GL_RGB;
    } else if (vecTiles[i]->band_count == 4) {
      rgbType = GL_RGBA;
    }
    if (vecTiles[i]->pixel_data) {
      glTexImage2D(GL_TEXTURE_2D, 0, rgbType, vecTiles[i]->tile.image_width,
                   vecTiles[i]->tile.image_height, 0, rgbType, GL_UNSIGNED_BYTE,
                   vecTiles[i]->pixel_data);
    }

    //����tile����������
    m_vecTileTexCoord.push_back(vecTiles[i]->tile);
  }
...
void RenderTileImageLayer::draw(RenderContext* rc) {
  // Mat4 view_matrix = rc->camera->view_matrix();
  /*
  �����޸ģ�
  ���� ��Ϊopengl matrix�������⣬��Ҫ������double���������֮���ٸ�opengl
  ������Ⱦ�����ź�ƽ��ʱ�����ڴ���������ֶ���
  */
  // view_matrix(0, 3) += m_localOrigin.x();
  // view_matrix(1, 3) += m_localOrigin.y();
  // glMatrixMode(GL_MODELVIEW);
  // glLoadMatrixd(view_matrix.data());

  // glTranslatef(x, y, 0);
  if (!m_loaderUpdateConnect) {
    QObject::connect(m_tileImageLoader, SIGNAL(send_update_tiles(const PyramidData*)), rc->widget,
...
void RenderTileImageLayer::receive_update_tiles(const PyramidData* p_data) {
  if (p_data->tiles.empty()) {
    return;
  }
  m_pyramidDataMutex.lock();
  m_paramidDeque.push_back(p_data);
  m_pyramidDataMutex.unlock();
}

void RenderTileImageLayer::repaint(RenderContext* rc, const QRectF& worldExtent) {
  int w, h, d;
  m_pyramid->get_whd(w, h, d);
  // to local extent
  //��rect��Χ���㵽��ǰ����ϵ
  //֧����ת
  Vec3 pts[4];
  ...
  // QRectF localExtent(worldExtent.x() - thisX, worldExtent.y() - thisY, worldExtent.width(),
  // worldExtent.height());
  ...
  VLOG(2) << "tile level=" << level << " scale=" << scale;
  if (level < 0)
    level = 0;
  if (level >= d) {
    level = d - 1;
  ...
  bool ok = m_pyramid->query_tiles(rect, level, tiles);
  if (ok && !tiles.empty()) {
    m_tileImageLoader->do_tasks(tiles);
  }
}

void RenderTileImageLayer::clearCache() {
  if (m_pyramid)
    m_pyramid->clear_all_tiles();
}
