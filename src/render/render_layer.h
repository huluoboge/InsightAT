...
class RENDER_EXPORT RenderLayer : public QObject, public RenderNode {
  Q_OBJECT
  Q_DISABLE_COPY(RenderLayer)
public:
  RenderLayer(QObject* parent) : QObject(parent) { m_visible = true; }
...
public slots:
  ...
  QRectF extent() const { return m_extent; }
...
class RENDER_EXPORT RenderTileImageLayer : public RenderLayer {
  Q_OBJECT
public:
...
public slots:
  ...
  void receive_update_tiles(const PyramidData* p_data);
  virtual void repaint(RenderContext* rc, const QRectF& worldExtent);
  void clearCache();
