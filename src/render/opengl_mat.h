...
#define MAT_TRANFORMS_TEMPLATE(MATCLASS, VECCLASS, VECSIZE) \
    ...
    { \
        ...
        static mat4 fromRowMaj(const T mat[16])
        {
            mat4 temp;
            for (int i = 0; i < 4; ++i)
            {
                for (int j = 0; j < 4; ++j)
                {
                    temp.elem[i][j] = mat[4 * i + j];
                }
            }
            return temp;
        }

        static mat4 fromColMaj(const T mat[16])
        {
            mat4 temp;
            for (int i = 0; i < 4; ++i)
            {
        ...
        //add by Jones.
        inline vec2<T> row2(int row) const
        {
            return MATRIX_ROW2((*this), row);
        }
        inline vec3<T> row3(int row) const
        {
            return MATRIX_ROW3((*this), row);
        }
        inline vec4<T> row4(int row) const
        {
            return MATRIX_ROW4((*this), row);
        }

        inline vec2<T> col2(int col) const
        {
            return MATRIX_COL2((*this), col);
        }

        inline vec3<T> col3(int col) const
        {
            return MATRIX_COL3((*this), col);
        }

        inline vec4<T> col4(int col) const
        {
            return MATRIX_COL4((*this), col);
        }
        ...
        inline void identity()
        {
            for (int r = 0; r < 4; ++r)
                for (int c = 0; c < 4; ++c)
                    elem[r][c] = (r == c) ? 1.0 : 0.0;
        }

        T *data() { return &elem[0][0]; }

        const T *data() const { return &elem[0][0]; }
    ...
    //add by jones.scale by pos
    template <typename T>
    inline mat4<T> scaling_pos_matrix(const T posx, const T posy, const T posz, const T x, const T y, const T z)
    {
        mat4<T> r(T(0));
        r.elem[0][0] = x;
        r.elem[1][1] = y;
    ...
    template <typename T>
    inline mat4<T> scaling_pos_matrix(const vec3<T>& pos, const vec3<T>& v)
    {
        return scaling_pos_matrix(pos.x, pos.y, pos.z, v.x, v.y, v.z);
    }
    ...
    template <typename T>
    inline mat3<T> rotation_R(const T angle, const vec3<T>& v)
    {
        const T a = angle * T(M_PI / 180);
        const vec3<T> u = normalize(v);
...
#undef FREE_OPERATORS
    ...
    inline Mat4 toMat4(const vmath::mat4<double> &vmat) {
        Mat4 mat;
        for (int r = 0; r < 4; ++r) {
            for (int c = 0; c < 4; ++c) {
                mat(r, c) = vmat.elem[r][c];
            }
        }
        return mat;
    }

    inline vmath::mat4<double> toVMat4(const Mat4 &mat) {
        vmath::mat4<double> vmat;
        for (int r = 0; r < 4; ++r) {
            for (int c = 0; c < 4; ++c) {
                vmat.elem[r][c] = mat(r, c);
            }
        }
        return vmat;
    }

    inline Mat4 fastInverse(const Mat4 &mat)
    {
        Vec3 t = mat.col(3).head<3>();
        double tx = -mat.col(0).head<3>().dot(t);
        double ty = -mat.col(1).head<3>().dot(t);
        double tz = -mat.col(2).head<3>().dot(t);

        Mat4 inverseMat;
        inverseMat <<
                      mat(0, 0), mat(1, 0), mat(2, 0), tx,
                mat(0, 1), mat(1, 1), mat(2, 1), ty,
                mat(0, 2), mat(1, 2), mat(2, 2), tz,
                0, 0, 0, 1;
        return inverseMat;
    }
