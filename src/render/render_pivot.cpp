...
namespace render {
void RenderPivot::gl_draw_unit_circle(int dim, unsigned steps /*= 64*/) {
...
RenderPivot::RenderPivot() : no_scale_(true) { model_matrix_ = Mat4::Identity(); }
...
void RenderPivot::draw(RenderContext* rc) {
  ...
  gl_draw_unit_circle(0, 128);

  glColor4f(0.0f, 1.0f, 0.0f, c_alpha);
  gl_draw_unit_circle(1, 128);

  glColor4f(0.0f, 0.7f, 1.0f, c_alpha);
  gl_draw_unit_circle(2, 128);
...
void RenderPivot::update_matrix(const Mat4& mat) {
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < 3; ++j) {
      model_matrix_(i, j) = mat(i, j);
    }
  }
}
