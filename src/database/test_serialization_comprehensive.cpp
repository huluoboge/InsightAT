 ...
 * @brief 完整数据库序列化单元测试（Cereal 与 optional/map/vector 等）。
 */
...
struct SimpleData {
  ...
  bool operator==(const SimpleData& other) const {
    return id == other.id && std::abs(value - other.value) < 1e-9;
  }
};
...
struct OptionalData {
  ...
  template <class Archive> void serialize(Archive& ar) { ar(opt_id, opt_value); }

  bool operator==(const OptionalData& other) const {
    return opt_id == other.opt_id && opt_value == other.opt_value;
  }
};
...
struct ComplexData {
  ...
  template <class Archive> void serialize(Archive& ar) { ar(items, mapping, optional_item); }
};
