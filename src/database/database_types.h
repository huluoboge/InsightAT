 ...
 * @brief InsightAT 数据库核心类型定义。
 *
 * 包含：CoordinateSystem、InputPose、Measurement、ATTask 等统一数据模型与序列化。
 */
 ...
 */
struct InputPose {
  enum class AngleUnit : int { kDegrees = 0, kRadians = 1 };
...
struct CameraModel {
  ...
  template <class Archive> void serialize(Archive& ar, std::uint32_t const version) {
    ...
    ar(CEREAL_NVP(sensor_width_mm), CEREAL_NVP(sensor_height_mm), CEREAL_NVP(pixel_size_um),
...
struct OptimizationConfig {
  ...
  template <class Archive> void serialize(Archive& ar, std::uint32_t const /*version*/) {
    ar(CEREAL_NVP(camera_optimization), CEREAL_NVP(enable_gnss_constraint), CEREAL_NVP(gnss_weight),
       CEREAL_NVP(max_reprojection_error), CEREAL_NVP(description));
  }
};
...
struct Project {
  ...
  // ── 缓存索引（懒加载）
  mutable std::map<uint32_t, std::vector<uint32_t>>
      image_to_gcp_cache;           ///< 图像ID -> GCP IDs（缓存）
