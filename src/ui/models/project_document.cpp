...
ProjectDocument::ProjectDocument(QObject* parent) : QObject(parent), m_projectLoaded(false) {}
...
void ProjectDocument::updateProjectInfo(const QString& name, const QString& author,
                                        const QString& description) {
  m_project.name = name.toStdString();
  m_project.author = author.toStdString();
  m_project.description = description.toStdString();
...
bool ProjectDocument::deleteImageGroup(uint32_t group_id) {
  auto it = std::find_if(m_project.image_groups.begin(), m_project.image_groups.end(),
...
bool ProjectDocument::removeImageFromGroup(uint32_t group_id, uint32_t image_id) {
  ...
  auto img_it = std::find_if(it->images.begin(), it->images.end(),
...
uint32_t ProjectDocument::createCameraRig(const QString& name, const QString& description) {
  if (!m_projectLoaded) {
    LOG(ERROR) << "No project loaded";
    return (uint32_t)-1;
  }
...
bool ProjectDocument::removeCameraFromRig(uint32_t rig_id, uint32_t camera_id) {
  ...
  auto mount_it = std::find_if(it->second.mounts.begin(), it->second.mounts.end(),
...
void ProjectDocument::updateRigCalibrationStatus(
    uint32_t rig_id, insight::database::CameraRig::CalibrationStatus status) {
  auto it = m_project.camera_rigs.find(rig_id);
...
bool ProjectDocument::exportToCOLMAP(const QString& outputDir,
...
bool ProjectDocument::importFromCOLMAP(const QString& colmapDb,
                                       const QMap<QString, QString>& options) {
  // TODO: 实现从COLMAP导入
...
void ProjectDocument::clearAllData() { m_project = insight::database::Project(); }
...
uint32_t ProjectDocument::generateImageId() { return m_project.next_image_id++; }
void ProjectDocument::syncCounters() {
  ...
  for (auto& group : m_project.image_groups) {
    if (group.group_id == 0 || group.group_id == static_cast<uint32_t>(-1) ||
        seenGroupIds.count(group.group_id)) {
  ...
  LOG(INFO) << "Counters synchronized and repaired. Next IDs: Image=" << m_project.next_image_id
            << ", Group=" << m_project.next_image_group_id << ", Rig=" << m_project.next_rig_id
            << ", GCP=" << m_project.next_gcp_id << ", AT_ID=" << m_project.next_at_task_id;
}
void ProjectDocument::applyGNSSToImages(
    const std::vector<database::Measurement::GNSSMeasurement>& gnssDataList, uint32_t groupId) {
...
uint32_t ProjectDocument::generateImageGroupId() { return m_project.next_image_group_id++; }

uint32_t ProjectDocument::generateRigId() { return m_project.next_rig_id++; }

uint32_t ProjectDocument::generateGCPId() { return m_project.next_gcp_id++; }
