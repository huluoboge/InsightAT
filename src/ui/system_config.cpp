...
bool UISystemConfig::load_coordinate_databases() {
  ...
  m_allCoordinates.insert(m_allCoordinates.end(), m_geoCoordinates.begin(), m_geoCoordinates.end());
  m_allCoordinates.insert(m_allCoordinates.end(), m_projCoordinates.begin(),
                          m_projCoordinates.end());

  m_loaded = true;
  LOG(INFO) << "Successfully loaded total " << m_allCoordinates.size() << " coordinate systems";
  return true;
}
...
std::vector<Coordinate> UISystemConfig::search_by_keyword(const std::string& keyword) const {
  ...
  for (const auto& coord : m_allCoordinates) {
    ...
    std::transform(lowerEPSGName.begin(), lowerEPSGName.end(), lowerEPSGName.begin(), ::tolower);
