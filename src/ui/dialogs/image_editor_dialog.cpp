...
#include "../widgets/gnss_measurement_import_dialog.h"
...
void ImageEditorDialog::InitializeUI() {
  ...
  connect(m_showGNSSDataButton, &QPushButton::toggled, this,
          &ImageEditorDialog::OnShowGNSSDataToggled);
  ...
  connect(m_setAllCovarianceButton, &QPushButton::clicked, this,
          &ImageEditorDialog::OnSetAllCovarianceClicked);
  connect(m_closeButton, &QPushButton::clicked, this, &ImageEditorDialog::OnCloseClicked);
  connect(m_imageTable, &QTableWidget::itemSelectionChanged, this,
...
void ImageEditorDialog::load_group(database::ImageGroup* group) {
  ...
  setWindowTitle(
      QString("Image Editor - Group: %1").arg(QString::fromStdString(m_currentGroup->group_name)));
...
void ImageEditorDialog::OnAddImagesClicked() {
  ...
  QString filter = "Image Files (*.jpg *.JPG *.jpeg *.JPEG *.png *.PNG *.tiff *.TIFF *.tif *.TIF "
                   "*.bmp *.BMP);;All Files (*)";
  QStringList filePaths =
      QFileDialog::getOpenFileNames(this, "Select Images", QDir::homePath(), filter);
...
void ImageEditorDialog::OnAddFolderClicked() {
  ...
  QDir dir(folderPath);
  dir.setNameFilters({"*.jpg", "*.JPG", "*.jpeg", "*.JPEG", "*.png", "*.PNG", "*.tiff", "*.TIFF",
                      "*.tif", "*.TIF", "*.bmp", "*.BMP"});
...
void ImageEditorDialog::OnCloseClicked() { close(); }
...
void ImageEditorDialog::OnTableSelectionChanged() {
  ...
  if (!selectedRows.isEmpty()) {
    ...
    if (m_currentGroup && m_selectedImageIndex >= 0 &&
        m_selectedImageIndex < static_cast<int>(m_currentGroup->images.size())) {
...
void ImageEditorDialog::OnImportGNSSClicked() {
  ...
  if (m_currentGroup->images.empty()) {
    QMessageBox::warning(this, "No Images",
                         "Please add images to the group before importing GNSS data");
    return;
  }

  // First, let user select a GNSS file
  QString gnssFilePath = QFileDialog::getOpenFileName(this, "Select GNSS Data File", "",
  ...
  gnssDialog.setWindowTitle("Import GNSS Measurements for: " +
                            QString::fromStdString(m_currentGroup->group_name));
  ...
  if (gnssDialog.exec() == QDialog::Accepted) {
    try {
      ...
      if (gnssData.size() != m_currentGroup->images.size()) {
        ...
        msg += "Only the first " +
               QString::number(std::min(gnssData.size(), m_currentGroup->images.size())) +
...
void ImageEditorDialog::OnGNSSDataChanged() { UpdateGNSSDetailsPanel(); }

std::vector<uint32_t> ImageEditorDialog::GetModifiedImages() const { return m_modifiedImages; }

void ImageEditorDialog::RefreshTable() { PopulateTable(); }
...
void ImageEditorDialog::OnSetAllCovarianceClicked() {
  ...
  QLabel* infoLabel = new QLabel(QString("Apply uniform covariance to %1 images in this group")
                                     .arg(m_currentGroup->images.size()),
                                 &covDialog);
  ...
  if (covDialog.exec() == QDialog::Accepted) {
    try {
      ...
      QMessageBox::critical(this, "Error",
                            QString("Failed to update covariance: %1").arg(e.what()));
    }
  }
}
