...
#include "image_group_dialog.h"
#include "../widgets/camera_model_widget.h"
...
ImageGroupDialog::ImageGroupDialog(insight::database::Project* project, QWidget* parent)
...
void ImageGroupDialog::initializeUI() {
  ...
  connect(m_cameraModelGroup, QOverload<int>::of(&QButtonGroup::idClicked), this,
          &ImageGroupDialog::onCameraModeChanged);

  connect(m_createButton, &QPushButton::clicked, this, &ImageGroupDialog::onCreateGroup);
...
void ImageGroupDialog::onCameraModeChanged() {
  ...
  // 获取相机配置组件的父组件
  QGroupBox* cameraGroup = qobject_cast<QGroupBox*>(m_cameraWidget->parentWidget()->parentWidget());
  QGroupBox* rigGroup = nullptr;

  // 查找 Rig 配置组件
  for (int i = 0; i < m_cameraWidget->parentWidget()
                          ->parentWidget()
                          ->parentWidget()
                          ->findChildren<QGroupBox*>()
                          .count();
       ++i) {
    QGroupBox* gb = m_cameraWidget->parentWidget()
                        ->parentWidget()
                        ->parentWidget()
                        ->findChildren<QGroupBox*>()[i];
...
void ImageGroupDialog::onCreateGroup() {
  ...
  case 1: // ImageLevel
    m_imageGroup.camera_mode = insight::database::ImageGroup::CameraMode::kImageLevel;
    m_imageGroup.group_camera.reset(); // 清空组级相机
    break;

  case 2: // RigBased
    m_imageGroup.camera_mode = insight::database::ImageGroup::CameraMode::kRigBased;
    // 从 combo 框获取 Rig 和 Mount 信息
    m_imageGroup.rig_mount_info = insight::database::ImageGroup::RigMountInfo();
