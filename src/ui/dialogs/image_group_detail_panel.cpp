/**
 * @file  ImageGroupDetailPanel.cpp
 * @brief 图像分组详情编辑对话框实现。
 */

#include "image_group_detail_panel.h"
...
ImageGroupDetailPanel::ImageGroupDetailPanel(QWidget* parent)
    : QDialog(parent), m_projectDocument(nullptr), m_currentGroup(nullptr) {
...
void ImageGroupDetailPanel::onCameraParameterModified() { SaveGroupData(); }
...
void ImageGroupDetailPanel::onGroupNameModified() { SaveGroupData(); }
...
void ImageGroupDetailPanel::onAutoEstimateRequested() {
  ...
  if (QFileInfo(QString::fromStdString(configPath)).isRelative()) {
    configPath =
        (qApp->applicationDirPath() + "/" + QString::fromStdString(configPath)).toStdString();
  }
  ...
  if (!process.waitForStarted()) {
    ...
    QMessageBox::critical(this, tr("Error"),
                          tr("Failed to start CameraEstimator algorithm at %1").arg(program));
    return;
  }
    ...
    if (res == QMessageBox::Yes) {
      ...
      for (int i = 0; i < groupsArray.size(); ++i) {
        ...
        if (i == 0) {
          ...
          uint32_t newGroupId = m_projectDocument->createImageGroup(
              newName, database::ImageGroup::CameraMode::kGroupLevel);
