/**
 * @file  GNSSMeasurementImportDialog.cpp
 * @brief GNSS 测量导入对话框实现。
 */

#include "gnss_measurement_import_dialog.h"
...
GNSSMeasurementImportDialog::GNSSMeasurementImportDialog(QWidget* parent)
    : GPSPointsWizardDialog(parent), m_useGeographic(true), m_importRotation(false) {
...
FieldConfiguration GNSSMeasurementImportDialog::get_field_configuration() const {
  ...
  // 可选字段
  config.optionalFields << "Uncertainty_X"
                        << "Uncertainty_Y"
                        << "Uncertainty_Z"
                        << "Covariance_XX"
                        << "Covariance_YY"
...
GNSSMeasurementImportDialog::get_gnss_measurements() {
  ...
  for (int rowIdx = rowFrom; rowIdx < doc->m_tableData.count(); ++rowIdx) {
    ...
    try {
      ...
      LOG(WARNING) << "Error parsing GNSS data at row " << (rowIdx + 1) << ": " << e.what();
      continue;
    }
  }
...
database::Measurement::GNSSMeasurement
GNSSMeasurementImportDialog::parseLine(const QStringList& fields,
  ...
  if (m_useGeographic) {
    ...
    if (latIdx >= 0 && lonIdx >= 0 && heightIdx >= 0 && latIdx < fields.count() &&
        lonIdx < fields.count() && heightIdx < fields.count()) {
    ...
    if (xIdx >= 0 && yIdx >= 0 && zIdx >= 0 && xIdx < fields.count() && yIdx < fields.count() &&
        zIdx < fields.count()) {
