/**
 * @file  CameraParameterEditorWidget.cpp
 * @brief 相机参数编辑器实现。
 */

#include "camera_parameter_editor_widget.h"
...
void CameraParameterEditorWidget::initialize_ui() {
  ...
  m_cameraParamsBox->setStyleSheet("QGroupBox { font-weight: bold; }");
  {
    ...
    // ImageLevel 模式占位符
    m_imageLevelPlaceholder =
        new QLabel(tr("Note: ImageLevel mode parameters are grayed out here.\n"
    ...
    // RigBased 模式占位符
    m_rigBasedPlaceholder =
        new QLabel(tr("Note: RigBased mode configuration is pending Rig editor implementation.\n"
  ...
  // 坐标系提示
  // ═══════════════════════════════════════════════════════════════
  m_coordinateSystemHintLabel = new QLabel(tr("Coordinate system starts from 0. "
...
void CameraParameterEditorWidget::load_camera(const database::CameraModel& camera) {
  ...
  m_focalLengthMmSpinBox->setValue(
      0.0); // No dedicated field for focal length mm in CameraModel yet?
