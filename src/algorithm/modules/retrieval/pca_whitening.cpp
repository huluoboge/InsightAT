/**
 * @file  pca_whitening.cpp
 * @brief PCA 模型读写与 CPU/CUDA 训练、单/批量应用实现。
 */

#include "pca_whitening.h"

#ifdef INSIGHTAT_USE_CUDA_PCA
...
PCAModel PCAModel::load(const std::string& filepath) {
  ...
  ifs.read(reinterpret_cast<char*>(model.components.data()), n_comp * in_dim * sizeof(float));

  // Read explained variance
  model.explained_variance.resize(n_comp);
  ifs.read(reinterpret_cast<char*>(model.explained_variance.data()), n_comp * sizeof(float));
...
bool PCAModel::save(const std::string& filepath) const {
  ...
  // Write mean vector
  ofs.write(reinterpret_cast<const char*>(mean.data()), input_dim * sizeof(float));
  ...
  ofs.write(reinterpret_cast<const char*>(explained_variance.data()), n_components * sizeof(float));
  ...
  LOG(INFO) << "Saved PCA model to " << filepath << " (" << (ofs.tellp() / 1024.0 / 1024.0)
            << " MB)";
...
PCAModel train_pca(const std::vector<float>& vlad_vectors, int num_samples, int input_dim,
  ...
  if (n_components > input_dim) {
    LOG(WARNING) << "n_components (" << n_components << ") > input_dim (" << input_dim
                 << "), clamping to input_dim";
    n_components = input_dim;
  }

  if (vlad_vectors.size() != static_cast<size_t>(num_samples * input_dim)) {
    LOG(ERROR) << "VLAD vectors size mismatch: expected " << (num_samples * input_dim) << ", got "
               << vlad_vectors.size();
    return model;
  }

  LOG(INFO) << "Training PCA: " << num_samples << " samples, " << input_dim << " dims -> "
            << n_components << " dims, "
            << "whiten=" << whiten;

#ifdef INSIGHTAT_USE_CUDA_PCA
  {
    std::vector<float> mean_out, components_out, explained_variance_out;
    if (train_pca_on_gpu(vlad_vectors.data(), num_samples, input_dim, n_components, whiten, mean_out,
                         components_out, explained_variance_out)) {
      model.mean = Eigen::Map<Eigen::VectorXf>(mean_out.data(), input_dim);
      model.components =
          Eigen::Map<Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>(
              components_out.data(), n_components, input_dim);
      model.explained_variance =
          Eigen::Map<Eigen::VectorXf>(explained_variance_out.data(), n_components);
      model.n_components = n_components;
      model.input_dim = input_dim;
      model.whiten = whiten;
      return model;
    }
    LOG(WARNING) << "PCA GPU path failed, falling back to CPU";
  }
#endif

  // Convert to Eigen matrix [num_samples x input_dim]
  Eigen::MatrixXf data =
      Eigen::Map<const Eigen::MatrixXf>(vlad_vectors.data(), input_dim, num_samples).transpose();
  ...
  LOG(INFO) << "PCA training complete: variance retained = " << (retention_ratio * 100.0f) << "%";
...
std::vector<float> apply_pca(const std::vector<float>& vlad, const PCAModel& model) {
  ...
  if (vlad.size() != static_cast<size_t>(model.input_dim)) {
    LOG(ERROR) << "VLAD dimension mismatch: expected " << model.input_dim << ", got "
               << vlad.size();
    return {};
  }
...
std::vector<float> apply_pca_batch(const std::vector<float>& vlad_vectors, int num_samples,
  ...
  for (int i = 0; i < num_samples; ++i) {
    std::vector<float> vlad(vlad_vectors.begin() + i * model.input_dim,
                            vlad_vectors.begin() + (i + 1) * model.input_dim);

    auto projected = apply_pca(vlad, model);
