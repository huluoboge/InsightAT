...
PCAModel trainPCA(
...
#ifdef INSIGHTAT_USE_CUDA_PCA
    {
      ...
      if (trainPCAOnGPU(vlad_vectors.data(), num_samples, input_dim, n_components, whiten,
                        mean_out, components_out, explained_variance_out)) {
        model.mean = Eigen::Map<Eigen::VectorXf>(mean_out.data(), input_dim);
        model.components = Eigen::Map<Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic,
                                                     Eigen::RowMajor>>(
