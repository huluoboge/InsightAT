/**
 * @file  spatial_retrieval.cpp
 * @brief 空间检索实现：nanoflann k-d 树、半径搜索、GNSS/IMU 过滤。
 */
...
std::vector<ImageInfo> filter_images_with_gnss(const std::vector<ImageInfo>& images) {
  ...
  LOG(INFO) << "Filtered " << result.size() << "/" << images.size() << " images with valid GNSS";
...
std::vector<ImagePair> retrieve_by_gps(const std::vector<ImageInfo>& images,
  ...
  auto valid_images = filter_images_with_gnss(images);
  ...
  auto all_neighbors =
      radius_search_batch(positions, options.distance_threshold, options.max_neighbors);
  ...
  for (size_t i = 0; i < valid_images.size(); ++i) {
    ...
    for (size_t j_idx : neighbors) {
      ...
      double distance = euclidean_distance(img1.gnss->position(), img2.gnss->position());

      if (options.use_imu_filter && img1.has_imu() && img2.has_imu() && options.angle_threshold > 0) {
        double angle_diff = angle_difference(img1.imu->attitude(), img2.imu->attitude());

        if (angle_diff > options.angle_threshold) {
          VLOG(2) << "Filtered pair (" << i << "," << j_idx << ") by angle: " << angle_diff << "°";
          continue;
        }
      }
      ...
      pair.score = compute_spatial_score(distance, options.distance_threshold);
      pair.method = "gps";
      pair.spatial_distance = distance;

      if (options.use_imu_filter && img1.has_imu() && img2.has_imu()) {
        pair.angle_difference = angle_difference(img1.imu->attitude(), img2.imu->attitude());
      }
  ...
  LOG(INFO) << "GPS retrieval: generated " << pairs.size() << " pairs from " << valid_images.size()
            << " images";
