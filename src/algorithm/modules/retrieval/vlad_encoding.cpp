/**
 * @file  vlad_encoding.cpp
 * @brief VLAD 编码实现：k-means（OpenCV）、聚类分配、VLAD 计算与缓存。
 */
...
std::vector<int> assign_to_clusters(const std::vector<float>& descriptors,
                                    const std::vector<float>& centroids, int descriptor_dim) {
...
std::vector<float> train_k_means(const std::vector<float>& descriptors, int num_clusters,
  ...
  if (num_descriptors < num_clusters) {
    LOG(WARNING) << "Too few descriptors (" << num_descriptors << ") for " << num_clusters
                 << " clusters";
    num_clusters = num_descriptors;
  }

  LOG(INFO) << "Training k-means: " << num_descriptors << " descriptors, " << num_clusters
            << " clusters, max_iter=" << max_iterations;
  ...
  cv::TermCriteria criteria(cv::TermCriteria::EPS + cv::TermCriteria::COUNT, max_iterations,
                            convergence_threshold);

  double compactness = cv::kmeans(samples, num_clusters, labels, criteria,
                                  3,                     // attempts
  ...
  if (centers.rows != num_clusters || centers.cols != descriptor_dim) {
    LOG(ERROR) << "Invalid centers dimensions: expected " << num_clusters << "x" << descriptor_dim
               << ", got " << centers.rows << "x" << centers.cols;
    return {};
  }
...
std::vector<float> encode_vlad(const std::vector<float>& descriptors,
  ...
  auto assignments = assign_to_clusters(descriptors, centroids, descriptor_dim);
...
float compute_scale_weight(float scale, float target_scale, float sigma) {
...
std::vector<float> extract_scales(const std::vector<float>& keypoints) {
  if (keypoints.size() % 4 != 0) {
    LOG(ERROR) << "Invalid keypoints size: " << keypoints.size() << " (must be multiple of 4)";
    return {};
  }
...
std::vector<float> encode_vlad_scale_weighted(const std::vector<float>& descriptors,
                                              ...
                                              const std::vector<float>& centroids, int num_clusters,
                                              float target_scale, float sigma) {
  ...
  if (static_cast<int>(scales.size()) != num_descriptors) {
    LOG(ERROR) << "Scale count mismatch: " << scales.size() << " vs " << num_descriptors
               << " descriptors";
    return {};
  }
  ...
  for (int i = 0; i < num_descriptors; ++i) {
    int cluster_id = assignments[i];
    float weight = compute_scale_weight(scales[i], target_scale, sigma);
...
bool save_vlad_cache(const std::string& cache_path, const std::vector<float>& vlad_vector) {
  ...
  // Write VLAD vector
  ofs.write(reinterpret_cast<const char*>(vlad_vector.data()), vlad_size * sizeof(float));
...
std::vector<float> load_or_compute_vlad(const std::string& feature_file,
                                        ...
                                        const std::vector<float>& centroids, int num_clusters,
                                        bool force_recompute, bool scale_weighted, float target_scale,
                                        float scale_sigma) {
  if (!force_recompute) {
    auto vlad = load_vlad_cache(cache_file);
    if (!vlad.empty()) {
      return vlad;
    }
  }
  ...
  if (scale_weighted) {
    ...
    vlad = encode_vlad_scale_weighted(descriptors, scales, centroids, num_clusters, target_scale,
                                      scale_sigma);
  } else {
    vlad = encode_vlad(descriptors, centroids, num_clusters);
  }

  if (!vlad.empty()) {
    if (!save_vlad_cache(cache_file, vlad)) {
