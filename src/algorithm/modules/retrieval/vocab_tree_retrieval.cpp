/**
 * @file  vocab_tree_retrieval.cpp
 * @brief DBoW3 词袋树检索实现；第三方 DBoW3 API（vocab_->load、database_->add 等）保持原样。
 */
...
bool VocabTreeRetriever::load_vocabulary(const std::string& vocab_file) {
  try {
    ...
    LOG(INFO) << "Vocabulary loaded: " << vocab_->size() << " words, "
...
bool VocabTreeRetriever::is_vocabulary_loaded() const { return !vocab_->empty(); }

int VocabTreeRetriever::add_image(int image_idx, const std::vector<float>& descriptors) {
...
std::vector<std::pair<int, float>> VocabTreeRetriever::query(const std::vector<float>& descriptors,
                                                             int max_results) {
  if (descriptors.empty()) {
    return {};
  }
...
void VocabTreeRetriever::clear_database() {
  database_->clear();
  image_indices_.clear();
}

int VocabTreeRetriever::get_database_size() const { return database_->size(); }
...
std::vector<ImagePair> retrieve_by_vocab_tree(const std::vector<ImageInfo>& images,
  ...
  if (!retriever.load_vocabulary(vocab_file)) {
  ...
  for (size_t i = 0; i < images.size(); ++i) {
    if (i < all_descriptors.size()) {
      retriever.add_image(i, all_descriptors[i]);
    }
  }

  LOG(INFO) << "Database built with " << retriever.get_database_size() << " images";
  ...
  std::sort(pairs.begin(), pairs.end(),
            [](const ImagePair& a, const ImagePair& b) { return a.score > b.score; });

  LOG(INFO) << "Vocabulary tree retrieval: generated " << pairs.size() << " pairs from "
            << images.size() << " images";
