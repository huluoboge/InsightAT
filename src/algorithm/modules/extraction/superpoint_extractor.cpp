/**
 * @file  superpoint_extractor.cpp
 * @brief SuperPoint 提取实现：ONNX 会话、前向、NMS。
 */
...
public:
  Impl(const SuperPointConfig& config) : config_(config) {
    ...
    LOG(INFO) << "SuperPoint model loaded: " << config_.model_path << ", inputs: " << num_input
              << ", outputs: " << num_output;
  }
      ...
      for (int x = 0; x < width; ++x) {
        if (final_mask.at<uchar>(y, x)) {
          ...
          if (desc_y < desc_h && desc_x < desc_w) {
            ...
            for (int c = 0; c < 256; ++c) {
              descriptor[c] = desc_data[c * desc_h * desc_w + desc_y * desc_w + desc_x];
            }
...
bool SuperPointExtractor::extract(const cv::Mat& image, std::vector<cv::KeyPoint>& keypoints,
                                  cv::Mat& descriptors, std::vector<float>* scores) {
  return impl_->extract(image, keypoints, descriptors, scores);
}

bool SuperPointExtractor::is_cuda_available() {
...
public:
  Impl(const SuperPointConfig&) {
    throw std::runtime_error(
        "SuperPoint not compiled in. Rebuild with INSIGHTAT_ENABLE_SUPERPOINT=ON and ONNXRuntime.");
  }
};
...
bool SuperPointExtractor::extract(const cv::Mat&, std::vector<cv::KeyPoint>&, cv::Mat&,
                                  std::vector<float>*) {
  return false;
}

bool SuperPointExtractor::is_cuda_available() { return false; }
