 ...
 * @brief 特征点分布策略实现：distribute_keypoints_grid、四叉树分布等。
 */
...
std::vector<size_t> distribute_keypoints_grid(const std::vector<SiftGPU::SiftKeypoint>& keypoints,
...
std::vector<size_t> distribute_keypoints_grid(const std::vector<cv::KeyPoint>& keypoints,
                                              int image_width, int image_height,
                                              const GridDistributionParams& params) {
  if (keypoints.empty())
    return {};
  ...
  for (auto& cell : grid_cells) {
    ...
    std::sort(cell.begin(), cell.end(), [&keypoints](size_t a, size_t b) {
...
std::vector<size_t> get_keypoint_indices(const std::vector<cv::KeyPoint>& original,
  ...
  for (const auto& kp : distributed) {
    int64_t key = (static_cast<int64_t>(kp.pt.x * 10) << 32) | static_cast<int64_t>(kp.pt.y * 10);
...
std::vector<size_t> distribute_keypoints_quadtree(const std::vector<cv::KeyPoint>& keypoints,
                                                  int image_width, int image_height,
  ...
  auto distributed =
      distribute_keypoints_via_tree(keypoints, 0, image_width, 0, image_height,
  ...
  return get_keypoint_indices(keypoints, distributed);
}
