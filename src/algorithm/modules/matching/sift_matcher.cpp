...
SiftMatcher::SiftMatcher(int max_features, bool use_cuda)
    ...
    // Request CUDA backend when available (before CreateContextGL so the matcher picks the right backend)
    if (use_cuda) {
...
MatchResult SiftMatcher::match(
    ...
    // Helper: build a sorted top-N index list for a FeatureData by scale (desc).
    // Returns indices into the original feature array.
    auto topNByScale = [](const FeatureData& f, int max_n) -> std::vector<int> {
        ...
        if (max_n > 0 && n > max_n) {
            std::partial_sort(idx.begin(), idx.begin() + max_n, idx.end(),
                [&f](int a, int b) {
                    return f.keypoints[a][2] > f.keypoints[b][2];  // desc scale
                });
            idx.resize(max_n);
        }
        return idx;  // identity (all features) when no capping needed
    };

    // Helper: upload a (possibly subsampled) FeatureData to the GPU slot.
    // Builds a compact descriptor buffer from the selected indices.
    auto uploadDescriptors = [&](int slot, const FeatureData& f, const std::vector<int>& idx) {
        ...
        if (f.descriptor_type == DescriptorType::kFloat32) {
            if (idx.size() == f.num_features) {
                ...
                matcher_->SetDescriptors(slot, cnt,
                    reinterpret_cast<const float*>(f.getDescriptorData()));
            } else {
                std::vector<float> buf(cnt * 128);
                for (int i = 0; i < cnt; ++i)
                    std::copy_n(f.descriptors_float.data() + idx[i] * 128, 128,
                                buf.data() + i * 128);
            ...
            if (idx.size() == f.num_features) {
                ...
                for (int i = 0; i < cnt; ++i)
                    std::copy_n(f.descriptors_uint8.data() + idx[i] * 128, 128,
                                buf.data() + i * 128);
                matcher_->SetDescriptors(slot, cnt, buf.data());
            }
        }
    ...
    // Select top-N features by scale (larger scale = more stable/distinctive)
    int cap = options.max_features_per_image;
    auto idx1 = topNByScale(features1, cap);
    auto idx2 = topNByScale(features2, cap);
    int n1 = static_cast<int>(idx1.size());
    int n2 = static_cast<int>(idx2.size());

    VLOG(1) << "Uploading descriptors: " << n1 << "/" << features1.num_features
            << " and " << n2 << "/" << features2.num_features
            << " (type=" << (features1.descriptor_type == DescriptorType::kFloat32 ? "f32" : "u8") << ")";

    // Upload to GPU (compact, subsampled if needed)
    uploadDescriptors(0, features1, idx1);
    uploadDescriptors(1, features2, idx2);
    ...
    // Execute matching
    VLOG(1) << "GetSiftMatch: max=" << max_match
    ...
    for (int i = 0; i < num_matches; ++i) {
        ...
        result.indices.emplace_back(static_cast<uint16_t>(orig1),
                                    static_cast<uint16_t>(orig2));
