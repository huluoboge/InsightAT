...
bool load_pair_geo(const std::string& geo_path,
  ...
  if (!meta.contains("twoview")) return false;
  ...
  *inlier_mask = reader.readBlob<uint8_t>("F_inliers");
  if (R_blob.size() != 9u || t_blob.size() != 3u || inlier_mask->empty()) return false;
  for (int i = 0; i < 3; ++i)
    for (int j = 0; j < 3; ++j)
      (*R)(i, j) = R_blob[static_cast<size_t>(i * 3 + j)];
  (*t)(0) = t_blob[0]; (*t)(1) = t_blob[1]; (*t)(2) = t_blob[2];
  return true;
}

bool load_pair_match(const std::string& match_path,
                     std::vector<uint16_t>* indices,
                     std::vector<float>* coords_pixel,
...
bool run_initial_pair_loop(const std::string& pairs_json_path,
                           ...
                           int min_tracks_after,
                           uint32_t* image1_id_out,
                           uint32_t* image2_id_out) {
  if (!store_out || !R1_out || !t1_out) return false;
  ViewGraph vg;
  if (!build_view_graph_from_geo(pairs_json_path, geo_dir, &vg)) return false;
  const auto pair = vg.choose_initial_pair({});
  if (!pair) return false;
  const uint32_t id1 = pair->first, id2 = pair->second;
  if (image1_id_out) *image1_id_out = id1;
  if (image2_id_out) *image2_id_out = id2;
  std::string dir_g = geo_dir;
  if (!dir_g.empty() && dir_g.back() != '/') dir_g += '/';
  std::string dir_m = match_dir;
  if (!dir_m.empty() && dir_m.back() != '/') dir_m += '/';
  const std::string geo_path = dir_g + std::to_string(id1) + "_" + std::to_string(id2) + ".isat_geo";
  const std::string match_path = dir_m + std::to_string(id1) + "_" + std::to_string(id2) + ".isat_match";

  Eigen::Matrix3d R;
  Eigen::Vector3d t;
  std::vector<uint8_t> inlier_mask;
  if (!load_pair_geo(geo_path, &R, &t, &inlier_mask)) return false;
  std::vector<uint16_t> indices;
  std::vector<float> coords_pixel, scales;
  if (!load_pair_match(match_path, &indices, &coords_pixel, &scales)) return false;
  if (inlier_mask.size() != indices.size() / 2u) return false;

  store_out->set_num_images(2);
  store_out->reserve_tracks(4096);
  store_out->reserve_observations(8192);
  const int n_added = build_tracks_from_pair_inliers(
      *store_out, 0, 1, inlier_mask, indices, coords_pixel, scales, nullptr);
  if (n_added < 8) return false;
  ...
  ba_in.R = R; ba_in.t = t;
  ba_in.fx = fx; ba_in.fy = fy; ba_in.cx = cx; ba_in.cy = cy;
  ...
  for (size_t ti = 0; ti < n_tracks; ++ti) {
    ...
    if (!store_out->is_track_valid(tid)) continue;
  ...
  for (size_t ti = 0; ti < store_out->num_tracks(); ++ti) {
    if (store_out->is_track_valid(static_cast<int>(ti))) ++n_valid;
  }
  if (n_valid < min_tracks_after) return false;
  *R1_out = R;
  *t1_out = t;
  return true;
}
...
int run_resection_loop(TrackStore* store,
  ...
  if (!store || !poses_R || !poses_t || !registered) return 0;
  const int n_images = store->num_images();
  if (n_images < 3) return 0;
  ...
  if (!(*registered)[0] || !(*registered)[1]) return 0;
  ...
  for (;;) {
    ...
    for (int tid : track_ids)
      if (store->track_has_triangulated_xyz(tid)) ++n_corr;
    ...
    Eigen::Matrix3d R_im;
    Eigen::Vector3d t_im;
    if (!resection_single_image(*store, im, fx, fy, cx, cy, &R_im, &t_im,
                                min_correspondences, 8.0, nullptr)) {
      skipped[static_cast<size_t>(im)] = true;
      continue;
    }
