...
TriangulateForCheirality(const Eigen::Vector2d& x1n, const Eigen::Vector2d& x2n,
  ...
  if (std::abs(X4[3]) < 1e-10)
    return {Eigen::Vector3d::Zero(), -1.0, -1.0};
...
StabilityMetrics ComputeStabilityMetrics(const std::vector<Eigen::Vector3d>& points3d,
                                         ...
                                         const Eigen::Matrix3d& R, const Eigen::Vector3d& t,
                                         double min_parallax_deg, double min_depth_baseline,
  ...
  if (N != pts1_n.size() || N != pts2_n.size() || N == 0)
    return out;

  const double baseline = t.norm();
  if (baseline < 1e-12)
    return out; // pure rotation, no baseline
