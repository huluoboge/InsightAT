...
int build_tracks_from_pair_inliers(TrackStore& store, int image1_index, int image2_index,
  ...
  for (int m = 0; m < num_matches; ++m) {
    ...
    store.add_observation(track_id, static_cast<uint32_t>(image1_index),
                          static_cast<uint32_t>(idx1), x1, y1, s1);
    store.add_observation(track_id, static_cast<uint32_t>(image2_index),
                          static_cast<uint32_t>(idx2), x2, y2, s2);
    ++inlier_idx;
    ++added;
  }
  return added;
