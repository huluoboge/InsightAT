...
{
    if (n <= s_cap_n) return;  // already large enough
...
int gpu_twoview_init(void)
{
    ...
    if (!s_prog_tri) return -1;

    s_prog_res = compileProg(injectDefine(RES_SHADER_TPL, s_local_size));
    if (!s_prog_res) { glDeleteProgram(s_prog_tri); s_prog_tri = 0; return -1; }
...
void gpu_twoview_shutdown(void)
{
    ...
    if (s_prog_tri) { glDeleteProgram(s_prog_tri); s_prog_tri = 0; }
    if (s_prog_res) { glDeleteProgram(s_prog_res); s_prog_res = 0; }
                      ...
                      int*         valid_out)
{
    ...
    if (valid > 0) wrss /= (float)valid;

    if (wrss_out)  *wrss_out  = wrss;
