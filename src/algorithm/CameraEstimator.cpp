...
struct EstimatorInput {
  ...
  template <class Archive> void serialize(Archive& ar) {
    ar(cereal::make_nvp("image_paths", image_paths),
       cereal::make_nvp("sensor_db_path", sensor_db_path), cereal::make_nvp("log_dir", log_dir));
  }
};
...
struct CameraResult {
  ...
  template <class Archive> void serialize(Archive& ar) {
    ar(cereal::make_nvp("make", make), cereal::make_nvp("model", model),
...
struct GroupResult {
  ...
  template <class Archive> void serialize(Archive& ar) {
    ar(cereal::make_nvp("camera", camera), cereal::make_nvp("image_indices", image_indices));
  }
};
...
struct EstimatorOutput {
  ...
  template <class Archive> void serialize(Archive& ar) { ar(cereal::make_nvp("groups", groups)); }
};
...
void PrintUsage(const char* prog) {
  ...
  std::cout << "--------------------------------\n";
  std::cout
      << "Estimates camera intrinsics from a list of images using EXIF and sensor databases.\n\n";
  ...
  std::cout << "Arguments:\n";
  std::cout << "  -j, --json-file PATH      Path to a JSON file containing the 'estimator_input' "
               "object.\n";
  std::cout << "  -c, --csv-file PATH       Path to a CSV file (one image path per line).\n";
  std::cout << "  -d, --db PATH             Path to the camera sensor database (replaces project "
               "default).\n";
...
int main(int argc, char* argv[]) {
  ...
  for (auto const& [key, indices] : groupedImages) {
    ...
    bool foundInDb =
        CameraSensorDatabase::instance().query_sensor_width(cam.make, cam.model, sensor_width);
    if (foundInDb) {
    ...
    if (meta.focal_35mm > 0.1f) {
      ...
      double diagonal_px =
          std::sqrt((double)cam.width * cam.width + (double)cam.height * cam.height);
    ...
    LOG(INFO) << "Estimated for group " << cam.make << " " << cam.model
              << ": f=" << cam.focal_length_px << "px";
  }
