                                                   ...
                                                   int target_group_id)
{
    ...
    if (all_groups) {
        for (const auto& group : groups) selected.push_back(&group);
        return selected;
    }
    ...
    if (!groups.empty()) selected.push_back(&groups[0]);
    return selected;
}
...
static int runCreate(int argc, char* argv[]) {
    ...
    printEvent({{"type","project.create"},
                {"ok",true},
                {"data", {{"project_path", project_file},
                          {"name", project.name},
                          {"uuid", project.uuid},
...
static int runAddGroup(int argc, char* argv[]) {
    ...
    cmd.add(make_option(0, camera_mode, "camera-mode").doc("Camera mode: group|image (default: group)"));
    std::string log_level;
    cmd.add(make_option(0, log_level, "log-level").doc("Log level: error|warn|info|debug"));
    cmd.add(make_switch('v', "verbose").doc("Verbose logging (INFO level)"));
    cmd.add(make_switch('q', "quiet").doc("Quiet mode (ERROR level only)"));
    ...
    if (!saveProjectToFile(project_file, project)) {
        printEvent({{"type","project.add_group"},{"ok",false},{"error","failed to write project"}});
        return 1;
    }
...
static int runAddImages(int argc, char* argv[]) {
    ...
    if (!fs::is_directory(input_dir)) {
        printEvent({{"type","project.add_images"},{"ok",false},{"error","input directory not found"}});
        return 1;
    }

    Project project;
    if (!loadProjectFromFile(project_file, project)) {
        printEvent({{"type","project.add_images"},{"ok",false},{"error","failed to load project"}});
        return 1;
    }
    ...
    auto addOne = [&](const fs::path& p) {
        ...
        if (dedup) existing_paths.insert(key);  // intra-batch dedup too
        added++;
    };
    ...
    if (recursive) {
        ...
        for (auto it = fs::directory_iterator(input_dir);
    ...
    project.last_modified_time = static_cast<int64_t>(std::time(nullptr));
    if (!saveProjectToFile(project_file, project)) {
        printEvent({{"type","project.add_images"},{"ok",false},{"error","failed to write project"}});
        return 1;
    }
...
static int runSetCamera(int argc, char* argv[]) {
    ...
    cmd.add(make_option(0, from_k_file, "from-k")
        .doc("Load fx/fy/cx/cy from isat_calibrate K.json (replaces --fx/--fy/--cx/--cy/--width/--height)"));
    ...
    if (!from_k_file.empty()) {
        ...
        try {
            ...
            cx = kj.value("cx", 0.0);
            cy = kj.value("cy", 0.0);
            if (kj.contains("width")  && width  == 0) width  = kj["width"];
            if (kj.contains("height") && height == 0) height = kj["height"];
        } catch (const std::exception& e) {
    ...
    Project project;
    if (!loadProjectFromFile(project_file, project)) {
        printEvent({{"type","project.set_camera"},{"ok",false},{"error","failed to load project"}});
        return 1;
    }
    ...
    if (!camera_name.empty()) cam.camera_name = camera_name;
    ...
    project.last_modified_time = static_cast<int64_t>(std::time(nullptr));
    if (!saveProjectToFile(project_file, project)) {
        printEvent({{"type","project.set_camera"},{"ok",false},{"error","failed to write project"}});
        return 1;
    }
...
static int runSetCS(int argc, char* argv[]) {
    ...
    cmd.add(make_option('p', project_file, "project").doc("Project file (.iat)"));
    cmd.add(make_option(0, type, "type").doc("Coordinate system type: local|enu|epsg|wkt (default: local)"));
    ...
    if (type == "enu") {
        ...
        if (have_ref) {
            ...
            ref.lat = lat; ref.lon = lon; ref.alt = alt;
            cs.reference = ref;
        }
    } else if (type == "epsg") {
    ...
    printEvent({{"type","project.set_cs"},
                {"ok",true},
                {"data", {{"project_path", project_file},
                          {"cs_type", type},
...
static int runInspect(int argc, char* argv[]) {
    ...
    cmd.add(make_option('t', task_id, "task-id")
        .doc("Optional: inspect specific AT task_id only"));
    std::string log_level;
    cmd.add(make_option(0, log_level, "log-level").doc("Log level: error|warn|info|debug"));
    cmd.add(make_switch('v', "verbose").doc("Verbose logging (INFO level)"));
    cmd.add(make_switch('q', "quiet").doc("Quiet mode (ERROR level only)"));
    ...
    Project project;
    if (!loadProjectFromFile(project_file, project)) return 1;
...
static int runCreateATTask(int argc, char* argv[]) {
    CmdLine cmd("Create an AT task with a snapshot of current project (image groups, measurements, CS). "
    ...
    Project project;
    if (!loadProjectFromFile(project_file, project)) {
        printEvent({{"type", "project.create_at_task"}, {"ok", false}, {"error", "failed to load project"}});
        return 1;
    }
    ...
    if (parent_task_id != static_cast<uint32_t>(-1)) {
        ...
        if (!parent) {
            ...
            printEvent({{"type", "project.create_at_task"}, {"ok", false}, {"error", "parent task not found"}});
            return 1;
        }
    }
    ...
    if (!saveProjectToFile(project_file, project)) {
        printEvent({{"type", "project.create_at_task"}, {"ok", false}, {"error", "failed to write project"}});
        return 1;
    }
...
static int runDeleteATTask(int argc, char* argv[]) {
    ...
    Project project;
    if (!loadProjectFromFile(project_file, project)) {
        printEvent({{"type", "project.delete_at_task"}, {"ok", false}, {"error", "failed to load project"}});
        return 1;
    }
    ...
    if (!saveProjectToFile(project_file, project)) {
        printEvent({{"type", "project.delete_at_task"}, {"ok", false}, {"error", "failed to write project"}});
        return 1;
    }
...
static int runExtract(int argc, char* argv[]) {
    ...
    cmd.add(make_option('o', output_file, "output")
        .doc("Output JSON for isat_extract input"));
    cmd.add(make_option('t', task_id, "task-id")
        .doc("AT task_id (required)"));
    ...
    cmd.add(make_switch('a', "all-groups")
        .doc("Export all groups"));
    cmd.add(make_switch(0, "no-gnss")
        .doc("Exclude GNSS fields"));
    cmd.add(make_switch(0, "no-imu")
        .doc("Exclude IMU fields"));
    std::string log_level;
    cmd.add(make_option(0, log_level, "log-level").doc("Log level: error|warn|info|debug"));
    cmd.add(make_switch('v', "verbose").doc("Verbose logging (INFO level)"));
    cmd.add(make_switch('q', "quiet").doc("Quiet mode (ERROR level only)"));
    ...
    for (const auto* group : selected) {
        for (const auto& image : group->images) {
            ...
            if (export_gnss && image.gnss_data.has_value()) {
                ...
                img["gnss"] = {
                    {"x", gnss.x}, {"y", gnss.y}, {"z", gnss.z},
                    {"cov_xx", gnss.cov_xx}, {"cov_yy", gnss.cov_yy}, {"cov_zz", gnss.cov_zz},
                    {"cov_xy", gnss.cov_xy}, {"cov_xz", gnss.cov_xz}, {"cov_yz", gnss.cov_yz},
                    {"num_satellites", gnss.num_satellites}, {"hdop", gnss.hdop}, {"vdop", gnss.vdop}
                };
                gnss_count++;
            }
            ...
            if (export_imu && image.input_pose.has_rotation) {
                ...
                img["imu"] = {
                    {"roll", omega}, {"pitch", phi}, {"yaw", kappa},
...
static const CameraModel* pickCameraFromTask(const ATTask& task,
                                             int group_id,
                                             int image_id,
                                             const ImageGroup** picked_group,
                                             const Image** picked_image)
{
    if (picked_group) *picked_group = nullptr;
    if (picked_image) *picked_image = nullptr;
    ...
    if (image_id >= 0) {
        for (const auto& group : groups) {
            if (group_id >= 0 && group.group_id != static_cast<uint32_t>(group_id)) continue;
            for (const auto& image : group.images) {
                if (image.image_id == static_cast<uint32_t>(image_id)) {
                    if (image.camera.has_value()) {
                        if (picked_group) *picked_group = &group;
    ...
    if (group_id >= 0) {
        for (const auto& group : groups) {
            if (group.group_id != static_cast<uint32_t>(group_id)) continue;
            if (group.group_camera.has_value()) {
                if (picked_group) *picked_group = &group;
                return &group.group_camera.value();
            }
...
static int runIntrinsics(int argc, char* argv[]) {
    CmdLine cmd("Export intrinsics JSON for isat_geo / isat_twoview from ATTask input snapshot.\n"
                "\n"
    ...
    if (cmd.used('a')) {
        ...
        for (const auto& group : groups) {
            ...
            const CameraModel* cam = pickCameraFromTask(
                *task,
                static_cast<int>(group.group_id),
                -1,    // no image filter
                &pg, &pi);

            if (!cam) {
                LOG(WARNING) << "Group " << group.group_id
                             << " (" << group.group_name << "): no camera model, skipping";
                continue;
            }
            ...
            // Optional metadata
            if (!cam->camera_name.empty()) cam_obj["camera_name"] = cam->camera_name;
            if (!cam->make.empty())        cam_obj["make"]        = cam->make;
            if (!cam->model.empty())       cam_obj["model"]       = cam->model;

            cameras_obj[std::to_string(group.group_id)] = cam_obj;
            ++exported;

            LOG(INFO) << "  Group " << group.group_id
                      << " (" << group.group_name << ")"
                      << "  fx=" << fx << " fy=" << fy
    ...
    const CameraModel* camera = pickCameraFromTask(*task, target_group_id, target_image_id,
                                                    &picked_group, &picked_image);
    if (!camera) {
    ...
    out["cx"] = cx;
    out["cy"] = cy;
    if (camera->width  > 0) out["width"]  = camera->width;
    if (camera->height > 0) out["height"] = camera->height;
    out["source"]["task_id"]   = task->task_id;
    out["source"]["task_uuid"] = task->id;
    out["source"]["task_name"] = task->task_name;
    if (picked_group) out["source"]["group_id"]  = picked_group->group_id;
    if (picked_image) out["source"]["image_id"]  = picked_image->image_id;
    ...
    LOG(INFO) << "Exported intrinsics to " << output_file
              << "  fx=" << fx << " fy=" << fy
              << " cx=" << cx << " cy=" << cy;
    return 0;
}
