                                           ...
                                           const std::unordered_map<uint32_t, uint32_t>& img_cam_map)
{
    ...
    if (!file.is_open()) LOG(FATAL) << "Cannot open pairs file: " << json_path;
    json j; file >> j;
    ...
    for (const auto& pair : j["pairs"]) {
        ...
        t.geo_file   = geo_dir   + "/" + std::to_string(t.image1_id) + "_" + std::to_string(t.image2_id) + ".isat_geo";
...
static void loadGeoAndMatch(TwoViewTask& task) {
    ...
    if (F_blob.size() == 9) std::copy(F_blob.begin(), F_blob.end(), task.F);
    else { task.F_ok = false; return; }
    ...
    if (task.E_ok) {
        ...
        if (E_blob.size() == 9) std::copy(E_blob.begin(), E_blob.end(), task.E);
        else task.E_ok = false;
    ...
    if (gm.contains("H")) {
        ...
        if (task.H_ok) {
            ...
            if (H_blob.size() == 9) std::copy(H_blob.begin(), H_blob.end(), task.H);
            else task.H_ok = false;
            if (task.H_ok) {
...
static void writeTwoView(const TwoViewTask& task, const std::string& output_dir) {
    ...
    std::string out = output_dir + "/" + std::to_string(task.image1_id) + "_" + std::to_string(task.image2_id) + ".isat_twoview";
    ...
    if (r.success && !r.points3d.empty()) {
        ...
        for (int i = 0; i < (int)r.points3d.size(); ++i) {
            if (!std::isfinite(r.points3d[i][0])) continue;
        ...
        if (!pts_flat.empty()) {
            ...
            writer.addBlob("points3d",   pts_flat.data(), pts_flat.size()*sizeof(double),
                           "float64", {Np, 3});
            writer.addBlob("inlier_ids", ids.data(), ids.size()*sizeof(int32_t),
                           "int32",   {Np});
        }
    }
...
static TwoViewResult reconstructPair(TwoViewTask& task,
                                      ...
                                      float  huber_k)
{
    ...
    if (n_inliers_src < min_points) {
        VLOG(1) << "  Pair " << task.image1_id << "-" << task.image2_id
                << ": only " << n_inliers_src << " inliers, skip";
        return result;
    }
    ...
    for (int i = 0; i < task.num_matches; ++i) {
        if (i >= (int)inlier_mask_src.size() || !inlier_mask_src[i]) continue;
    ...
    if ((int)pts1_px_inl.size() < min_points) {
        VLOG(1) << "  Pair " << task.image1_id << "-" << task.image2_id
                << ": collected " << pts1_px_inl.size() << " inliers (< " << min_points << ")";
        return result;
    }
    ...
    if (have_K) {
        ...
        if (f <= 0.0) {
            LOG(WARNING) << "  FocalFromFundamental failed for pair "
    ...
    if (have_K) {
        for (int i = 0; i < Np_all; ++i) {
            pts1_n[i] = {(pts1_px_inl[i][0] - K1.cx) / K1.fx,
                         (pts1_px_inl[i][1] - K1.cy) / K1.fy};
            pts2_n[i] = {(pts2_px_inl[i][0] - K2.cx) / K2.fx,
                         (pts2_px_inl[i][1] - K2.cy) / K2.fy};
        }
    } else {
        for (int i = 0; i < Np_all; ++i) {
    ...
    t_f[0] = (float)t[0];  t_f[1] = (float)t[1];  t_f[2] = (float)t[2];
    ...
    float wrss = 0.0f;
    int valid = 0;
    gpu_ba_residuals(pts_px_flat.data(), X_gpu.data(), Np,
                     R_f, t_f,
                     static_cast<float>(f),
    ...
    const StabilityMetrics stability = ComputeStabilityMetrics(
        X_init, pts1_n, pts2_n, R, t, 2.0, 2.0, 500.0);
    result.quality = (result.reprojection_rmse < 1.5 && stability.is_stable)
                         ? "good"
                         : "poor";
    return result;
}
...
int main(int argc, char* argv[])
{
    ...
    cmd.add(make_option('g', geo_dir, "geo-dir")
        .doc("Directory of .isat_geo files (from isat_geo)"));
    ...
    cmd.add(make_option('o', output_dir, "output")
        .doc("Output directory for .isat_twoview files"));
    ...
    cmd.add(make_option('j', num_threads, "threads")
        .doc("CPU threads for I/O stages. Default: 4"));
    ...
    if (!have_K)
        ...
        LOG(INFO) << "Loaded " << cam_intrinsics.size() << " camera(s), "
                  << img_cam_map.size() << " images mapped.";

    // ── Setup ────────────────────────────────────────────────────────────────
    if (!fs::is_directory(geo_dir))
        { LOG(ERROR) << "geo-dir not found: " << geo_dir; return 1; }
    if (!fs::is_directory(match_dir))
        { LOG(ERROR) << "match-dir not found: " << match_dir; return 1; }
    ...
    Stage loadStage("LoadGeoMatch", num_threads, IO_Q,
        [&](int i) { loadGeoAndMatch(tasks[i]); });

    // ── Stage 2: GPU triangulate + residuals  (main thread, EGL context) ─────────
    StageCurrent reconStage("ReconTV", 1, GPU_Q,
        [&](int i) {
            ...
            task.recon = reconstructPair(task, K1, K2, f_hint,
                                          min_points, static_cast<float>(ba_loss_k));
            task.processed = true;

            auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::high_resolution_clock::now() - t0).count();
            LOG(INFO) << "Recon [" << i << "/" << total << "] "
                      << task.image1_id << "–" << task.image2_id
                      << (task.recon.success
                          ? ("  f=" + std::to_string((int)task.recon.focal_refined)
    ...
    Stage writeStage("WriteTV", num_threads, IO_Q,
        [&](int i) {
            if (!tasks[i].processed) return;
    ...
    std::thread push_thread([&]() {
        for (int i = 0; i < total; ++i) loadStage.push(i);
    });
    ...
    for (const auto& t : tasks) {
        if (!t.processed || !t.recon.success) { ++n_fail; continue; }
        if (t.recon.quality == "good") ++n_good; else ++n_poor;
        if (t.recon.focal_refined > 0.0) { f_sum += t.recon.focal_refined; ++f_cnt; }
    }
