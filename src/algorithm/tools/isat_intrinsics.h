...
inline CameraIntrinsicsMap loadIntrinsicsMap(const std::string& path)
{
    ...
    if (j.contains("schema") &&
        j["schema"].get<std::string>() == "multi_camera_v1")
    {
        // ── Multi-camera format ───────────────────────────────────────────────
        ...
        for (auto& [key, val] : j["cameras"].items()) {
            uint32_t id = 1;
            try { id = static_cast<uint32_t>(std::stoul(key)); }
            catch (...) {
    ...
    else
    {
        ...
        result[1] = K;
        LOG(INFO) << "Loaded single-camera intrinsics from " << path
                  << ": fx=" << K.fx << " fy=" << K.fy
                  << " cx=" << K.cx << " cy=" << K.cy;
    }
                                             ...
                                             uint32_t camera_id)
{
    ...
    if (it != cam_map.end()) return &it->second;
    if (!cam_map.empty()) return &cam_map.begin()->second;
    return nullptr;
}
 ...
 */
inline std::unordered_map<uint32_t, uint32_t>
buildImageCameraMap(const std::string& path)
{
    std::unordered_map<uint32_t, uint32_t> map;
    if (path.empty()) return map;
