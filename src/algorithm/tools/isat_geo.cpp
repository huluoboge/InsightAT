...
/// Minimum inlier count to still write inlier mask to IDC for track building (degenerate pairs keep inlier data)
...
static std::vector<GeoTask> loadPairs(
    ...
    const std::unordered_map<uint32_t, uint32_t>& img_cam_map)
{
    ...
    for (const auto& pair : j["pairs"]) {
        ...
        t.match_file = match_dir + "/" + std::to_string(t.image1_id) + "_" + std::to_string(t.image2_id) + ".isat_match";
        t.index = idx++;

        // Per-pair camera IDs from image list (image_id is numeric string)
        try {
            auto it1 = img_cam_map.find(t.image1_id);
            if (it1 != img_cam_map.end()) t.camera1_id = it1->second;
            auto it2 = img_cam_map.find(t.image2_id);
            if (it2 != img_cam_map.end()) t.camera2_id = it2->second;
...
static float sampsonSq(const float F[9], float x1, float y1, float x2, float y2) {
    ...
    float num = F[6]*x1*x2 + F[7]*y1*x2 + F[8]*x2
              + F[3]*x1*y2 + F[4]*y1*y2 + F[5]*y2
              + F[0]*x1    + F[1]*y1    + F[2];
    // Use direct formulation: num = (x2, y2, 1) · F · (x1, y1, 1)
    float r = (F[0]*x1 + F[1]*y1 + F[2])*x2
            + (F[3]*x1 + F[4]*y1 + F[5])*y2
            + (F[6]*x1 + F[7]*y1 + F[8]);
...
static std::vector<uint8_t> computeFMask(const float F[9],
...
static std::vector<uint8_t> computeHMask(const float H[9],
                                          const std::vector<float>& coords,
                                          int n, float thresh_sq)
{
    std::vector<uint8_t> mask(n, 0);
...
static std::vector<uint8_t> computeEMask(const float E[9],
                                          const std::vector<float>& coords,
                                          int n,
...
static void writeGeo(const GeoTask& task, const std::string& output_dir,
                     int ransac_iter)
{
    std::string out = output_dir + "/" + std::to_string(task.image1_id) + "_" + std::to_string(task.image2_id) + ".isat_geo";
    ...
    gm["F"]["inlier_ratio"] = (task.num_matches > 0)
        ? static_cast<float>(task.F_inliers) / task.num_matches : 0.0f;

    gm["E"]["estimated"]  = task.E_ok;
    gm["E"]["num_inliers"] = task.E_inliers;
    gm["E"]["inlier_ratio"] = (task.num_matches > 0)
        ? static_cast<float>(task.E_inliers) / task.num_matches : 0.0f;

    gm["H"]["estimated"]  = task.H_ok;
    gm["H"]["num_inliers"] = task.H_inliers;
    gm["H"]["inlier_ratio"] = (task.num_matches > 0)
        ? static_cast<float>(task.H_inliers) / task.num_matches : 0.0f;
    ...
    if (task.F_ok)
        writer.addBlob("F_matrix", task.F, 9 * sizeof(float),
                       "float32", {3, 3});
    if (static_cast<int>(task.F_mask.size()) == task.num_matches)
        writer.addBlob("F_inliers", task.F_mask.data(), task.num_matches,
                       "uint8", {task.num_matches});
    if (task.E_ok)
        writer.addBlob("E_matrix", task.E, 9 * sizeof(float),
                       "float32", {3, 3});
    if (static_cast<int>(task.E_mask.size()) == task.num_matches)
        writer.addBlob("E_inliers", task.E_mask.data(), task.num_matches,
                       "uint8", {task.num_matches});
    if (task.H_ok)
        writer.addBlob("H_matrix", task.H, 9 * sizeof(float),
                       "float32", {3, 3});
    if (static_cast<int>(task.H_mask.size()) == task.num_matches)
        writer.addBlob("H_inliers", task.H_mask.data(), task.num_matches,
                       "uint8", {task.num_matches});
    if (task.twoview_ok) {
        writer.addBlob("R_matrix", task.R, 9 * sizeof(float),
                       "float32", {3, 3});
        writer.addBlob("t_vector", task.t, 3 * sizeof(float),
                       "float32", {3});
        writer.addBlob("points3d", task.points3d.data(),
                       task.points3d.size() * sizeof(float),
...
static void writeMatchMatrix(const std::vector<GeoTask>& tasks,
                              ...
                              InlierFn inlier_fn)
{
    ...
    for (int i = 0; i < N; i++) id_idx[ids[static_cast<size_t>(i)]] = i;
    ...
    for (const auto& t : tasks) {
        ...
        if (count > max_inliers) max_inliers = count;
        pairs_ok++;
    }

    if (pairs_ok == 0) {
        LOG(WARNING) << "No valid " << model_label
                     << " pairs to visualise, skipping " << output_path;
        return;
    }

    // Normalise to [0, 255], then apply VIRIDIS colormap
    cv::Mat norm8;
    mat.convertTo(norm8, CV_8U,
                  max_inliers > 0 ? 255.0 / max_inliers : 0.0);
    ...
    cv::putText(legend, std::to_string(max_inliers),
                {cb_w + 4, 14}, cv::FONT_HERSHEY_SIMPLEX, 0.4,
                cv::Scalar(200, 200, 200), 1);
    cv::putText(legend, "0",
                {cb_w + 4, cb_h - 4}, cv::FONT_HERSHEY_SIMPLEX, 0.4,
                cv::Scalar(200, 200, 200), 1);
    // Model label at top
    cv::putText(legend, model_label,
                {cb_w + 4, 30}, cv::FONT_HERSHEY_SIMPLEX, 0.4,
    ...
    if (!cv::imwrite(output_path, canvas)) {
        ...
        LOG(WARNING) << "Match graph [" << model_label << "] saved: " << output_path
                     << "  (" << N << "x" << N << " images"
                     << ", pairs=" << pairs_ok
...
static std::string makeVisPath(const std::string& base_path,
...
int main(int argc, char* argv[])
{
    ...
    cmd.add(make_option('o', output_dir, "output")
        .doc("Output directory for .isat_geo files"));
    cmd.add(make_option('k', intrinsics_json, "intrinsics")
    ...
    cmd.add(make_switch(0, "vis")
        .doc("Generate adjacency matrix heatmap images (PNG) per model.\n"
    ...
    LOG(INFO) << "  Estimate E   : " << (estimate_E ? "yes" : "no")
              << (estimate_E ? " (" + std::to_string(cam_intrinsics.size()) + " camera(s) loaded)" : " (-k not provided)");
    if (estimate_E)
        LOG(INFO) << "  Image list   : " << image_list_json
                  << "  (" << img_cam_map.size() << " images mapped)";
    ...
    Stage loadStage("LoadMatches", num_threads, IO_Q,
        [&tasks](int i) {
            ...
            VLOG(2) << "  [load] pair " << i << " → "
                    << tasks[i].num_matches << " matches  " << ms << "ms";
        });

    // ── Stage 2: GPU RANSAC (single thread = main thread, EGL context) ───────
    StageCurrent geoStage("GeoEstimate", 1, GPU_Q,
        [&](int i) {
            GeoTask& task = tasks[i];

            if (task.num_matches < 8) {
                LOG(WARNING) << "Pair [" << i << "] " << task.image1_id
                             << "–" << task.image2_id
            ...
            // ── Estimate F ───────────────────────────────────────────────────
            task.F_inliers = gpu_ransac_F(pts.data(), task.num_matches,
                                          task.F, thresh_f_sq);
            task.F_ok = (task.F_inliers >= min_inliers);
            if (task.F_ok || task.F_inliers >= kMinInliersForTrack)
                task.F_mask = computeFMask(task.F, task.coords,
                                           task.num_matches, thresh_f_sq);
            ...
            if (estimate_E) {
                ...
                if (!pK1 || !pK2) {
                    LOG(WARNING) << "Pair [" << i << "] "
                                 ...
                                 << ": cannot find K for camera"
                                 << (!pK1 ? task.camera1_id : task.camera2_id)
                                 << ", skipping E";
                } else {
                    ...
                    if (task.camera1_id != task.camera2_id) {
                        VLOG(1) << "  Pair [" << i << "] cross-camera: "
                                << task.camera1_id << " x " << task.camera2_id;
                    }
                    ...
                    task.E_inliers = gpu_ransac_E(pts_norm.data(), task.num_matches,
                                                  task.E, thresh_e_sq);
                    task.E_ok = (task.E_inliers >= min_inliers);
                    if (task.E_ok || task.E_inliers >= kMinInliersForTrack)
                        task.E_mask = computeEMask(task.E, task.coords,
                                                   task.num_matches, K1, K2, thresh_e_sq);
                }
            }

            // ── Estimate H (if --estimate-h) ─────────────────────────────────
            if (estimate_H) {
                task.H_inliers = gpu_ransac_H(pts.data(), task.num_matches,
                                              task.H, thresh_h_sq);
                task.H_ok = (task.H_inliers >= min_inliers);
                if (task.H_ok || task.H_inliers >= kMinInliersForTrack)
                    task.H_mask = computeHMask(task.H, task.coords,
                                               task.num_matches, thresh_h_sq);
            }

            // ── Degeneracy detection (F vs H) ──────────────────────────────────
            task.degeneracy = DetectDegeneracy(task.F_inliers,
                                              task.H_ok ? task.H_inliers : 0,
                                              task.num_matches);

            auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::high_resolution_clock::now() - t0).count();

            LOG(INFO) << "Geo [" << i << "/" << total << "] "
                      << task.image1_id << "–" << task.image2_id
                      << "  n=" << task.num_matches
                      << "  F=" << task.F_inliers
                      << (estimate_E  ? ("  E=" + std::to_string(task.E_inliers)) : "")
                      << (estimate_H  ? ("  H=" + std::to_string(task.H_inliers)) : "")
                      << (task.degeneracy.is_degenerate ? "  [DEG]" : "")
                      << "  " << ms << "ms";
    ...
    Stage writeStage("WriteGeo", num_threads, IO_Q,
        [&](int i) {
        ...
        if (gpu_twoview_init() != 0) {
            ...
            for (int i = 0; i < total; ++i) {
                ...
                for (int k = 0; k < task.num_matches; k++) {
                    ...
                    pts1_n.emplace_back(
                        (task.coords[k*4+0] - K1.cx) / K1.fx,
                        (task.coords[k*4+1] - K1.cy) / K1.fy);
                    pts2_n.emplace_back(
                        (task.coords[k*4+2] - K2.cx) / K2.fx,
                        (task.coords[k*4+3] - K2.cy) / K2.fy);
                }
                if ((int)pts1_n.size() < 8) continue;
                ...
                float Rf[9], tf[3];
                for (int r = 0; r < 3; r++)
                    for (int c = 0; c < 3; c++) Rf[r*3+c] = static_cast<float>(R_mat(r,c));
                ...
                task.stability = ComputeStabilityMetrics(
                    points3d_eigen, pts1_valid, pts2_valid, R_mat, t_vec);
    ...
    int pairs_tv = 0;
    if (run_twoview) {
        for (const auto& t : tasks) if (t.twoview_ok) pairs_tv++;
    }
    ...
    for (const auto& t : tasks) {
        summary_pairs.push_back({
            {"image1_id", t.image1_id},
    ...
    if (estimate_E) data["pairs_with_E"] = pairs_E;
    if (estimate_H) data["pairs_with_H"] = pairs_H;
    if (run_twoview) data["pairs_twoview_ok"] = pairs_tv;
    ...
    if (cmd.used("vis")) {
        const std::string vis_base = vis_output.empty()
