 ...
 *
 * Usage:
 *   isat_retrieve -i image_list.json -f feat_dir/ -o pairs.json --strategy exhaustive
 *   isat_retrieve -i image_list.json -f feat_dir/ -o pairs.json --strategy vlad -c codebook.bin
 */
...
ImageInfo convertToRetrievalFormat(const LegacyImageInfo& legacy, const json& metadata = {}) {
  ImageInfo info;
  try {
    info.image_id = static_cast<uint32_t>(std::stoul(legacy.image_id));
  } catch (...) {
    info.image_id = 0;
  }
...
std::vector<ImageInfo> loadImagesFromFeatures(const std::string& feature_dir) {
  ...
  for (const auto& entry : fs::directory_iterator(feature_dir)) {
    ...
    info.image_id =
        static_cast<uint32_t>(images.size()); // 0-based index when loading from directory
  ...
  std::sort(images.begin(), images.end(),
            [](const ImageInfo& a, const ImageInfo& b) { return a.image_id < b.image_id; });
...
std::vector<ImageInfo> loadImagesFromJSON(const std::string& json_path,
  ...
  int gnss_count =
      std::count_if(images.begin(), images.end(), [](const ImageInfo& i) { return i.has_gnss(); });
  int imu_count =
      std::count_if(images.begin(), images.end(), [](const ImageInfo& i) { return i.has_imu(); });
 ...
 */
std::vector<ImagePair> retrieveExhaustive(const std::vector<ImageInfo>& images,
 ...
 */
std::vector<ImagePair> retrieveSequential(const std::vector<ImageInfo>& images,
                                          const RetrievalOptions& options) {
  std::vector<ImagePair> pairs;
  int num_images = images.size();
...
const std::map<std::string, RetrievalFunction> STRATEGIES = {
    {"exhaustive", retrieveExhaustive}, {"sequential", retrieveSequential}, {"gps", retrieve_by_gps}
 ...
 */
bool writePairsJSON(const std::vector<ImageInfo>& images, const std::vector<ImagePair>& pairs,
                    const std::string& output_path, const std::string& retrieval_method) {
  ...
  for (const auto& p : pairs) {
    if (!p.is_valid() || p.image1_idx >= static_cast<int>(images.size()) ||
...
int main(int argc, char* argv[]) {
  ...
  cmd.add(make_option('o', output_file, "output").doc("Output pairs file (JSON format)"));
  ...
  cmd.add(make_option('s', strategy, "strategy")
              .doc("Strategy: exhaustive|sequential|gps|vlad|gps+sequential|gps+vlad (default: "
                   "exhaustive)"));
  ...
  if (vlad_enabled) {
    ...
    if (!pca_model_file.empty()) {
      ...
      LOG(INFO) << "Loaded PCA model: " << pca_model.input_dim << " -> " << pca_model.n_components
                << " dimensions";
    }
  }
  ...
  if (vlad_enabled) {
    ...
    const PCAModel* pca_ptr = pca_model.is_valid() ? &pca_model : nullptr;

    // Capture scale weighting parameters
    bool use_scale_weighting = cmd.used("vlad-scale-weighted");

    strategies["vlad"] = [&vlad_centroids, cache_path, pca_ptr, use_scale_weighting, target_scale,
                          scale_sigma](const std::vector<ImageInfo>& imgs,
                                       const RetrievalOptions& opts) -> std::vector<ImagePair> {
      return retrieve_by_vlad(imgs, opts, vlad_centroids, cache_path, pca_ptr, use_scale_weighting,
  ...
  if (strategy_names.size() == 1) {
    ...
    pairs = combine_pairs(all_pairs, true);
    LOG(INFO) << "Combined " << strategy_names.size() << " strategies -> " << pairs.size()
              << " unique pairs";
  }
  ...
  pairs = filter_pairs(pairs, [](const ImagePair& p) { return p.score > 0.01; });
  pairs = sort_by_score(pairs);
  if (options.max_pairs > 0 && pairs.size() > static_cast<size_t>(options.max_pairs)) {
  ...
  LOG(INFO) << "Coverage: " << pairs.size() << "/" << total_possible << " (" << coverage << "%)";

  // Write output
  if (!writePairsJSON(images, pairs, output_file, strategy)) {
    printEvent(
        {{"type", "retrieve.complete"}, {"ok", false}, {"error", "failed to write pairs JSON"}});
    return 1;
  }

  printEvent({{"type", "retrieve.complete"},
              {"ok", true},
              {"data",
               {{"num_images", static_cast<int>(images.size())},
                {"num_pairs", static_cast<int>(pairs.size())},
