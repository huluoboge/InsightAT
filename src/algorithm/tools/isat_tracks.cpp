 ...
 *   Phase 2  Fill observations (coords + scales) from match + geo inlier masks; set xyz from geo
 * points3d when available Write    Single .isat_tracks IDC for downstream SfM
 *
 * Usage:
...
static std::vector<uint32_t> get_image_ids(const std::string& image_list_path,
  ...
  if (!image_list_path.empty()) {
    ...
    for (const auto& img : j["images"]) {
      if (img.contains("id"))
        ids.insert(img["id"].get<uint32_t>());
    }
...
static bool save_track_store_to_idc(const TrackStore& store, const std::vector<uint32_t>& image_ids,
...
static bool load_track_store_from_idc(const std::string& path, TrackStore* store,
  ...
  for (int t = 0; t < num_tracks; ++t) {
    ...
    for (size_t g = beg; g < end; ++g) {
      ...
      store->add_observation(t, obs_image_id[g], obs_feature_id[g], obs_u[g], obs_v[g], s);
    }
  }
...
int main(int argc, char* argv[]) {
  ...
  cmd.add(make_option('l', image_list, "image-list")
              .doc("Image list JSON for stable image ordering (optional)"));
  ...
  if (stats_only) {
    TrackStore store;
    std::vector<uint32_t> image_ids;
    if (!load_track_store_from_idc(output_path, &store, &image_ids))
      return 1;
