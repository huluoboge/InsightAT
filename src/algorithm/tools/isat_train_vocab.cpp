/**
 * isat_train_vocab.cpp
 * InsightAT Vocabulary Tree Training Tool (DBoW3)
 *
 * Samples descriptors from .isat_feat files, builds a hierarchical
 * vocabulary tree (k-means at each level) and saves it in .dbow3 format
 * for use with isat_retrieve --strategy vocab_tree.
 *
 * Usage:
 *   isat_train_vocab -f feat_dir/ -o vocab.dbow3 -k 10 -L 5
 */
 ...
 */
std::vector<cv::Mat> sampleDescriptorsMultiFile(const std::vector<std::string>& feature_files,
  ...
  for (const auto& file : feature_files) {
    ...
    if (num_features <= max_descriptors_per_file) {
      ...
      for (int i = 0; i < max_descriptors_per_file; ++i) {
        int idx = indices[i];
        std::memcpy(desc_mat.ptr<float>(i), &descriptors[idx * 128], 128 * sizeof(float));
      }
      all_descriptors.push_back(desc_mat);
...
int main(int argc, char* argv[]) {
  ...
  cmd.add(make_option('o', output_file, "output").doc("Output vocabulary file (.dbow3 format)"));

  // Optional parameters
  cmd.add(make_option('k', branching_factor, "branching")
              .doc("Branching factor for k-means (default: 10)"));
  cmd.add(make_option('L', depth, "depth").doc("Tree depth (default: 6, gives k^L words)"));
  ...
  printEvent({{"type", "train_vocab.complete"},
              {"ok", true},
              {"data",
               {{"output_file", output_file},
                {"vocab_words", static_cast<int>(vocab.size())},
                {"branching_factor", vocab.getBranchingFactor()},
                {"depth", vocab.getDepthLevels()},
