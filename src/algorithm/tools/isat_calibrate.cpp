...
static bool loadTvHeader(CalibEntry& e) {
    ...
    if (!rdr.isValid()) return false;
    const auto& meta = rdr.getMetadata();
    if (!meta.value("success", false)) return false;
...
static bool loadTvFull(CalibEntry& e) {
    ...
    if (!tv_rdr.isValid()) return false;
    ...
    if (ids_blob.size() != (size_t)Np) return false;

    // Load pixel coords from .isat_match
    if (e.match_path.empty()) return false;
    IDCReader match_rdr(e.match_path);
    if (!match_rdr.isValid()) return false;
    ...
    for (int i = 0; i < Np; ++i) {
        ...
        if (idx < 0 || idx >= N_all) return false;
...
static double evalFocal(std::vector<CalibEntry>& entries,
...
static double weightedMedian(const std::vector<double>& vals,
...
int main(int argc, char* argv[])
{
    ...
    for (const auto& de : fs::directory_iterator(twoview_dir)) {
        if (de.path().extension() != ".isat_twoview") continue;
        CalibEntry e;
        e.twoview_path = de.path().string();
        if (!loadTvHeader(e)) continue;
        if (e.reprojection_rmse > rmse_thresh) continue;
        if (!match_dir.empty()) {
            e.match_path = match_dir + "/" + std::to_string(e.image1_id) + "_" + std::to_string(e.image2_id) + ".isat_match";
        }
        entries.push_back(std::move(e));
    }

    LOG(WARNING) << "Found " << entries.size() << " usable pairs (rmse < "
                 << rmse_thresh << " px)";

    // ── Phase 1: weighted median focal estimate ────────────────────────────
    std::vector<double> focals, weights;
    for (const auto& e : entries) {
        if (e.focal_estimate <= 0.0) continue;
        // Weight by quality proxy: more points + lower rmse → higher weight
        double w = static_cast<double>(e.n_points)
                 / (1.0 + e.reprojection_rmse * e.reprojection_rmse);
        // Quality flag
        if (e.quality == "good") w *= 2.0;
        focals.push_back(e.focal_estimate);
        weights.push_back(w);
    }

    if ((int)focals.size() < min_pairs) {
        LOG(ERROR) << "Not enough good pairs (" << focals.size()
                   << " < " << min_pairs << "). "
    ...
    LOG(WARNING) << "Phase-1 weighted-median focal = " << f0 << " px  ("
                 << focals.size() << " pairs)";
    ...
    if (n_loaded >= min_pairs) {
        ...
        LOG(WARNING) << "Phase-2 refined focal = " << f_best << " px  (rmse="
                     << rmse_best << " px)";
    } else {
    ...
    if (!out_file.is_open()) {
        ...
        printEvent({{"type", "calibrate.complete"}, {"ok", false}, {"error", "cannot open output: " + output_json}});
        return 1;
    }
    out_file << K.dump(2) << "\n";
    printEvent({
