...
public:
  ...
  Exif_IO_EasyExif(const std::string& sFileName) : bHaveExifInfo_(false) { open(sFileName); }
  ...
  bool open(const std::vector<unsigned char>& buf) {
    if (buf.size() > 0) {
      // Parse EXIF
      int code = exifInfo_.parseFrom(buf.data(), buf.size());
      if (code)
        bHaveExifInfo_ = false;
      else
        bHaveExifInfo_ = true;
  ...
  size_t getWidth() const { return exifInfo_.ImageWidth; }

  size_t getHeight() const { return exifInfo_.ImageHeight; }

  float getFocal() const { return static_cast<float>(exifInfo_.FocalLength); }

  float getFocal35mm() const { return static_cast<float>(exifInfo_.FocalLengthIn35mm); }
  ...
  std::string getLensModel() const { return ""; }

  /**Verify if the file has metadata*/
  bool doesHaveExifInfo() const { return bHaveExifInfo_; }

  /** Print all data*/
  std::string allExifData() const {
    std::ostringstream os;
    os << "Camera make       : " << exifInfo_.Make << "Camera model      : " << exifInfo_.Model
       ...
       << "Subsecond time    : " << exifInfo_.SubSecTimeOriginal << "Exposure time     : 1/time "
       << (unsigned)(1.0 / exifInfo_.ExposureTime) << "F-stop            : " << exifInfo_.FNumber
       << "ISO speed         : " << exifInfo_.ISOSpeedRatings
       << "Subject distance  : " << exifInfo_.SubjectDistance << "Exposure bias     : EV"
       << exifInfo_.ExposureBiasValue << "Flash used?       : " << exifInfo_.Flash
       << "Metering mode     : " << exifInfo_.MeteringMode << "Lens focal length : mm\n"
       << exifInfo_.FocalLength << "35mm focal length : mm\n"
       << exifInfo_.FocalLengthIn35mm << "GPS Latitude      : deg ( deg, min, sec )\n"
  ...
  const EXIFInfo exifInfo() const { return exifInfo_; }
