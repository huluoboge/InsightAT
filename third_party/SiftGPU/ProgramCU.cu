...
////////////////////////////////////////////////////////////////////////////
// =========================================================================
// CUDA 12.x migration notes (April 2026)
// =========================================================================
// All legacy texture-reference APIs (texture<>, cudaBindTexture, tex1Dfetch
// with implicit reference, tex2D with implicit reference) have been replaced
// with the bindless texture-object API (cudaTextureObject_t).
//
// Every kernel that previously read through a global `texture<>` variable
// now receives one or more `cudaTextureObject_t` parameters instead.
// The host-side wrapper creates the texture object via
// CuTexImage::CreateTextureObject(), passes it to the kernel, and destroys
// it afterwards via CuTexImage::DestroyTextureObject().
//
// `cudaThreadSynchronize()` → `cudaDeviceSynchronize()`.
// Legacy CUDA-GL interop helpers are handled in CuTexImage.cpp.
// =========================================================================
...
// saturate() was a device built-in in CUDA ≤11.x but removed in CUDA 12.x.
// __CUDACC_VER_MAJOR__ is always defined by nvcc; CUDA_VERSION requires <cuda.h>.
#if defined(__CUDACC_VER_MAJOR__) && __CUDACC_VER_MAJOR__ >= 12
__device__ __forceinline__ float saturate(float x) { return fminf(fmaxf(x, 0.0f), 1.0f); }
#endif
...
//////////////////////////////////////////////////////////////
// FilterH – horizontal Gaussian filter
//////////////////////////////////////////////////////////////
template<int FW> __global__ void FilterH(float* d_result, int width, cudaTextureObject_t texObj)
{
...
#pragma unroll
	for(int j = 0; j < CACHE_COUNT; ++j)
	{
		if(cache_index < CACHE_WIDTH)
		{
			...
			data[cache_index] = tex1Dfetch<float>(texObj, fetch_index);
...
////////////////////////////////////////////////////////////////////
// FilterV – vertical Gaussian filter
////////////////////////////////////////////////////////////////////
template<int FW> __global__ void FilterV(float* d_result, int width, int height, cudaTextureObject_t texObj)
{
	const int HALF_WIDTH = FW >> 1;
...
#pragma unroll
		for(int i = 0; i < CACHE_COUNT; ++i)
		{
			if(cache_col_start < CACHE_WIDTH - i * FILTERV_BLOCK_HEIGHT)
			{
				...
				data[cache_index + i * FILTERV_BLOCK_HEIGHT] = tex1Dfetch<float>(texObj, fetch_index);
...
#pragma unroll
			...
			{
				value += (data[index_start + fi] * d_kernel[fi]);
			}
...
// Upsample kernel
//////////////////////////////////////////////////////////////
template<int LOG_SCALE> __global__ void UpsampleKernel(float* d_result, int width, cudaTextureObject_t texObj)
{
	...
	if (helper)
	{
		float v11 = tex1Dfetch<float>(texObj, index);
		float v12 = tex1Dfetch<float>(texObj, index + 1);
		index += width;
		float v21 = tex1Dfetch<float>(texObj, index);
		float v22 = tex1Dfetch<float>(texObj, index + 1);
...
#pragma unroll
	...
	{
		float v1 = tex1Dfetch<float>(texObj, index);
		float v2 = tex1Dfetch<float>(texObj, index + 1);
		d_result[dst_idx] = v1;
#pragma unroll
		for(int i = 1; i < SCALE; ++i)
		{
...
void ProgramCU::SampleImageU(CuTexImage *dst, CuTexImage *src, int log_scale)
{
	...
	cudaTextureObject_t texObj = src->CreateTextureObject();
	dim3 grid((width +  FILTERH_TILE_WIDTH - 1)/ FILTERH_TILE_WIDTH, height << log_scale);
	dim3 block(FILTERH_TILE_WIDTH);
	switch(log_scale)
	{
	case 1 : UpsampleKernel<1><<<grid, block>>>((float*)dst->_cuData, width, texObj); break;
	case 2 : UpsampleKernel<2><<<grid, block>>>((float*)dst->_cuData, width, texObj); break;
	case 3 : UpsampleKernel<3><<<grid, block>>>((float*)dst->_cuData, width, texObj); break;
	default: break;
	}
	src->DestroyTextureObject(texObj);
}

//////////////////////////////////////////////////////////////
// Downsample kernels
//////////////////////////////////////////////////////////////
template<int LOG_SCALE> __global__ void DownsampleKernel(float* d_result, int src_width, int dst_width, cudaTextureObject_t texObj)
{
	...
	d_result[dst_idx] = tex1Dfetch<float>(texObj, src_idx);
}

__global__ void DownsampleKernelGeneric(float* d_result, int src_width, int dst_width, const int log_scale, cudaTextureObject_t texObj)
{
	const int dst_col = IMUL(blockIdx.x, FILTERH_TILE_WIDTH) + threadIdx.x;
	if(dst_col >= dst_width) return;
	const int src_col = min((dst_col << log_scale), (src_width - 1));
...
void ProgramCU::SampleImageD(CuTexImage *dst, CuTexImage *src, int log_scale)
{
	...
	switch(log_scale)
	{
	case 1 : DownsampleKernel<1><<<grid, block>>>((float*)dst->_cuData, src_width, dst_width, texObj); break;
	case 2 : DownsampleKernel<2><<<grid, block>>>((float*)dst->_cuData, src_width, dst_width, texObj); break;
	case 3 : DownsampleKernel<3><<<grid, block>>>((float*)dst->_cuData, src_width, dst_width, texObj); break;
	default: DownsampleKernelGeneric<<<grid, block>>>((float*)dst->_cuData, src_width, dst_width, log_scale, texObj);
	}
	src->DestroyTextureObject(texObj);
}

//////////////////////////////////////////////////////////////
// Channel reduce / convert
//////////////////////////////////////////////////////////////
__global__ void ChannelReduce_Kernel(float* d_result, cudaTextureObject_t texObj)
{
	int index = IMUL(blockIdx.x, FILTERH_TILE_WIDTH) + threadIdx.x;
	d_result[index] = tex1Dfetch<float>(texObj, index*4);
}

__global__ void ChannelReduce_Convert_Kernel(float* d_result, cudaTextureObject_t texObj)
{
	int index = IMUL(blockIdx.x, FILTERH_TILE_WIDTH) + threadIdx.x;
	float4 rgba = tex1Dfetch<float4>(texObj, index);
...
void ProgramCU::ReduceToSingleChannel(CuTexImage* dst, CuTexImage* src, int convert_rgb)
{
	...
	cudaTextureObject_t texObj = src->CreateTextureObject();
	if(convert_rgb)
		ChannelReduce_Convert_Kernel<<<grid, block>>>((float*)dst->_cuData, texObj);
	else
		ChannelReduce_Kernel<<<grid, block>>>((float*)dst->_cuData, texObj);
	src->DestroyTextureObject(texObj);
}

__global__ void ConvertByteToFloat_Kernel(float* d_result, cudaTextureObject_t texObj)
{
	int index = IMUL(blockIdx.x, FILTERH_TILE_WIDTH) + threadIdx.x;
	d_result[index] = tex1Dfetch<float>(texObj, index);
}
...
void ProgramCU::ConvertByteToFloat(CuTexImage*src, CuTexImage* dst)
{
	...
	dim3 block(FILTERH_TILE_WIDTH);
	cudaTextureObject_t texObj = src->CreateTextureObject();
	ConvertByteToFloat_Kernel<<<grid, block>>>((float*)dst->_cuData, texObj);
	src->DestroyTextureObject(texObj);
}
...
//////////////////////////////////////////////////////////////
// FilterImage – separable Gaussian (H then V)
//////////////////////////////////////////////////////////////
template<int FW> void ProgramCU::FilterImage(CuTexImage *dst, CuTexImage *src, CuTexImage* buf)
{
	...
	// horizontal filtering
	{
		...
		FilterH<FW><<<gridh, blockh>>>((float*)buf->_cuData, width, texObj);
		src->DestroyTextureObject(texObj);
	}
	...
	// vertical filtering
	{
		cudaTextureObject_t texObj = buf->CreateTextureObject();
		...
		FilterV<FW><<<gridv, blockv>>>((float*)dst->_cuData, width, height, texObj);
		buf->DestroyTextureObject(texObj);
	}
...
void __global__ ComputeDOG_Kernel(float* d_dog, float2* d_got, int width, int height,
                                  cudaTextureObject_t texCObj, cudaTextureObject_t texPObj)
{
	...
	if(col < width && row < height)
	{
		...
		float vp = tex1Dfetch<float>(texPObj, index);
		float v = tex1Dfetch<float>(texCObj, index);
		d_dog[index] = v - vp;
		float vxn = tex1Dfetch<float>(texCObj, index + 1);
		float vxp = tex1Dfetch<float>(texCObj, index - 1);
		float vyp = tex1Dfetch<float>(texCObj, index - width);
		float vyn = tex1Dfetch<float>(texCObj, index + width);
...
void __global__ ComputeDOG_Kernel_NoGot(float* d_dog, int width, int height,
                                        cudaTextureObject_t texCObj, cudaTextureObject_t texPObj)
{
	int row = (blockIdx.y << DOG_BLOCK_LOG_DIMY) + threadIdx.y;
	int col = (blockIdx.x << DOG_BLOCK_LOG_DIMX) + threadIdx.x;
...
void ProgramCU::ComputeDOG(CuTexImage* gus, CuTexImage* dog, CuTexImage* got)
{
	...
	cudaTextureObject_t texCObj = gus->CreateTextureObject();
	cudaTextureObject_t texPObj = (gus-1)->CreateTextureObject();
	if(got->_cuData)
		ComputeDOG_Kernel<<<grid, block>>>((float*)dog->_cuData, (float2*)got->_cuData, width, height, texCObj, texPObj);
	else
		ComputeDOG_Kernel_NoGot<<<grid, block>>>((float*)dog->_cuData, width, height, texCObj, texPObj);
	gus->DestroyTextureObject(texCObj);
	(gus-1)->DestroyTextureObject(texPObj);
}

//////////////////////////////////////////////////////////////
// Keypoint detection
//////////////////////////////////////////////////////////////
#define READ_CMP_DOG_DATA(datai, texObj, idx) \
		datai[0] = tex1Dfetch<float>(texObj, idx - 1);\
		datai[1] = tex1Dfetch<float>(texObj, idx);\
		datai[2] = tex1Dfetch<float>(texObj, idx + 1);\
		if(v > nmax)\
		{\
...
void __global__ ComputeKEY_Kernel(float4* d_key, int width, int colmax, int rowmax,
					...
					cudaTextureObject_t texCObj, cudaTextureObject_t texPObj, cudaTextureObject_t texNObj)
{
	float data[3][3], v;
	...
	{
		in_image = 1;
		data[1][1] = v = tex1Dfetch<float>(texCObj, idx[1]);
		if(fabs(v) <= dog_threshold0) goto key_finish;

		data[1][0] = tex1Dfetch<float>(texCObj, idx[1] - 1);
		data[1][2] = tex1Dfetch<float>(texCObj, idx[1] + 1);
		nmax = max(data[1][0], data[1][2]);
		nmin = min(data[1][0], data[1][2]);

		if(v <=nmax && v >= nmin) goto key_finish;
		READ_CMP_DOG_DATA(data[0], texCObj, idx[0]);
		READ_CMP_DOG_DATA(data[2], texCObj, idx[2]);
		...
		READ_CMP_DOG_DATA(datap[0], texPObj, idx[0]);
		READ_CMP_DOG_DATA(datap[1], texPObj, idx[1]);
		READ_CMP_DOG_DATA(datap[2], texPObj, idx[2]);

		// read the next level
		READ_CMP_DOG_DATA(datan[0], texNObj, idx[0]);
		READ_CMP_DOG_DATA(datan[1], texNObj, idx[1]);
		READ_CMP_DOG_DATA(datan[2], texNObj, idx[2]);
...
void ProgramCU::ComputeKEY(CuTexImage* dog, CuTexImage* key, float Tdog, float Tedge)
{
	...
	cudaTextureObject_t texPObj = dogp->CreateTextureObject();
	cudaTextureObject_t texCObj = dog->CreateTextureObject();
	cudaTextureObject_t texNObj = dogn->CreateTextureObject();
	...
	ComputeKEY_Kernel<<<grid, block>>>((float4*)key->_cuData, width,
		...
		texCObj, texPObj, texNObj);
	dogp->DestroyTextureObject(texPObj);
	dog->DestroyTextureObject(texCObj);
	dogn->DestroyTextureObject(texNObj);
}

//////////////////////////////////////////////////////////////
// Histogram init / reduce / list generation
//////////////////////////////////////////////////////////////
void __global__ InitHist_Kernel(int4* hist, int ws, int wd, int height, cudaTextureObject_t texKeyF4)
{
...
#pragma unroll
			...
			{
				float4 temp = tex1Dfetch<float4>(texKeyF4, sidx + i);
...
void ProgramCU::InitHistogram(CuTexImage* key, CuTexImage* hist)
{
	...
	cudaTextureObject_t texKeyF4 = key->CreateTextureObject();
	InitHist_Kernel<<<grid, block>>>((int4*)hist->_cuData, ws, wd, hd, texKeyF4);
	key->DestroyTextureObject(texKeyF4);
}

void __global__ ReduceHist_Kernel(int4* d_hist, int ws, int wd, int height, cudaTextureObject_t texHistI4)
{
	int row = IMUL(blockIdx.y, blockDim.y) + threadIdx.y;
	int col = IMUL(blockIdx.x, blockDim.x) + threadIdx.x;
	if(row < height && col < wd)
...
#pragma unroll
		...
		{
			int4 temp = tex1Dfetch<int4>(texHistI4, sidx + i);
...
void ProgramCU::ReduceHistogram(CuTexImage*hist1, CuTexImage* hist2)
{
	...
	dim3 block(BW, BH);
	ReduceHist_Kernel<<<grid, block>>>((int4*)hist2->_cuData, ws, wd, hd, texHistI4);
	hist1->DestroyTextureObject(texHistI4);
}

void __global__ ListGen_Kernel(int4* d_list, int list_len, int width,
                               cudaTextureObject_t texListObj, cudaTextureObject_t texHistI4)
{
	int idx1 = IMUL(blockIdx.x, blockDim.x) + threadIdx.x;
	int4 pos = tex1Dfetch<int4>(texListObj, idx1);
	int idx2 = IMUL(pos.y, width) + pos.x;
	int4 temp = tex1Dfetch<int4>(texHistI4, idx2);
...
void ProgramCU::GenerateList(CuTexImage* list, CuTexImage* hist)
{
	...
	list->DestroyTextureObject(texListObj);
	hist->DestroyTextureObject(texHistI4);
}
...
void __global__ ComputeOrientation_Kernel(float4* d_list,
										  ...
										  int subpixel,
										  int keepsign,
										  cudaTextureObject_t texF4Obj,
										  cudaTextureObject_t texListObj,
										  cudaTextureObject_t texF2Obj)
{
	...
	{
		key = tex1Dfetch<float4>(texF4Obj, idx);
	}else
	{
		int4 ikey = tex1Dfetch<int4>(texListObj, idx);
		key.x = ikey.x + 0.5f;
		key.y = ikey.y + 0.5f;
		key.z = sigma;
		if(subpixel || keepsign)
		{
			float4 offset = tex1Dfetch<float4>(texF4Obj, IMUL(width, ikey.y) + ikey.x);
			if(subpixel)
			{
				key.x += offset.y;
				key.y += offset.z;
...
#pragma unroll
	...
	for(float y = ymin; y <= ymax; y += 1.0f)
	{
		for(float x = xmin; x <= xmax; x += 1.0f)
		{
			...
			float2 got = tex2D<float2>(texF2Obj, x, y);
								   ...
								   float sigma, float sigma_step, int existing_keypoint)
{
	...
	cudaTextureObject_t texF4Obj = 0;
	...
	ComputeOrientation_Kernel<<<grid, block>>>((float4*)list->_cuData,
		...
		texF4Obj, texListObj, texF2Obj);

	ProgramCU::CheckErrorCUDA("ComputeOrientation");

	if(texF4Obj) {
		if(existing_keypoint) list->DestroyTextureObject(texF4Obj);
		else key->DestroyTextureObject(texF4Obj);
	}
	if(texListObj) list->DestroyTextureObject(texListObj);
	got->DestroyTextureObject(texF2Obj);
}
...
template <bool DYNAMIC_INDEXING> void __global__ ComputeDescriptor_Kernel(float4* d_des, int num,
											 ...
											 cudaTextureObject_t texF4Obj, cudaTextureObject_t texF2Obj)
{
	const float rpi = 4.0/ 3.14159265358979323846;
	int idx = IMUL(blockIdx.x, blockDim.x) + threadIdx.x;
	int fidx = idx >> 4;
	if(fidx >= num) return;
	float4 key = tex1Dfetch<float4>(texF4Obj, fidx);
...
#pragma unroll
	...
	for(float y = ymin; y <= ymax; y += 1.0f)
	{
		for(float x = xmin; x <= xmax; x += 1.0f)
		{
			...
			{
				float2 cc = tex2D<float2>(texF2Obj, x, y);
				...
				if(DYNAMIC_INDEXING)
				{
					des[fi] += (weight1 * weight);
					des[fi + 1] += (weight2 * weight);
				}else
				{
					#pragma unroll
					for(int k = 0; k < 8; ++k)
...
void __global__ NormalizeDescriptor_Kernel(float4* d_des, int num, cudaTextureObject_t texDesF4)
{
	float4 temp[32];
	int idx = IMUL(blockIdx.x, blockDim.x) + threadIdx.x;
	if(idx >= num) return;
...
#pragma unroll
	for(int i = 0; i < 32; ++i)
	{
		temp[i] = tex1Dfetch<float4>(texDesF4, sidx + i);
...
void ProgramCU::ComputeDescriptor(CuTexImage*list, CuTexImage* got, CuTexImage* dtex, int rect, int stream)
{
	...
	cudaTextureObject_t texF2Obj = got->CreateTextureObject2D();
	cudaTextureObject_t texF4Obj = list->CreateTextureObject();
	...
	if(rect)
	{
		if(GlobalUtil::_UseDynamicIndexing)
			ComputeDescriptorRECT_Kernel<true><<<grid, block>>>((float4*)dtex->_cuData, num, width, height, GlobalUtil::_DescriptorWindowFactor, texF4Obj, texF2Obj);
		else
			ComputeDescriptorRECT_Kernel<false><<<grid, block>>>((float4*)dtex->_cuData, num, width, height, GlobalUtil::_DescriptorWindowFactor, texF4Obj, texF2Obj);
	}else
	{
		if(GlobalUtil::_UseDynamicIndexing)
			ComputeDescriptor_Kernel<true><<<grid, block>>>((float4*)dtex->_cuData, num, width, height, GlobalUtil::_DescriptorWindowFactor, texF4Obj, texF2Obj);
		else
			ComputeDescriptor_Kernel<false><<<grid, block>>>((float4*)dtex->_cuData, num, width, height, GlobalUtil::_DescriptorWindowFactor, texF4Obj, texF2Obj);
	}
	list->DestroyTextureObject(texF4Obj);
	got->DestroyTextureObject(texF2Obj);

	if(GlobalUtil::_NormalizedSIFT)
	{
		cudaTextureObject_t texDesF4 = dtex->CreateTextureObject();
		const int bw = DESCRIPTOR_NORMALIZ_PER_BLOCK;
		dim3 ngrid((num + bw -1) / bw);
		dim3 nblock(bw);
		NormalizeDescriptor_Kernel<<<ngrid, nblock>>>((float4*)dtex->_cuData, num, texDesF4);
		dtex->DestroyTextureObject(texDesF4);
	}
...
{
	// cudaThreadSynchronize() was removed in CUDA 12.x
#if defined(__CUDACC_VER_MAJOR__) && __CUDACC_VER_MAJOR__ >= 12
...
//////////////////////////////////////////////////////////////
void __global__ ConvertDOG_Kernel(float* d_result, int width, int height, cudaTextureObject_t texObj)
{
	...
	{
		int index = row * width + col;
		float v = tex1Dfetch<float>(texObj, index);
...
void ProgramCU::DisplayConvertDOG(CuTexImage* dog, CuTexImage* out)
{
	...
	int width = dog->GetImgWidth(), height = dog->GetImgHeight();
	cudaTextureObject_t texObj = dog->CreateTextureObject();
	dim3 grid((width + BLOCK_DIM - 1)/ BLOCK_DIM, (height + BLOCK_DIM - 1)/BLOCK_DIM);
	dim3 block(BLOCK_DIM, BLOCK_DIM);
	ConvertDOG_Kernel<<<grid, block>>>((float*)out->_cuData, width, height, texObj);
	dog->DestroyTextureObject(texObj);
	ProgramCU::CheckErrorCUDA("DisplayConvertDOG");
}

void __global__ ConvertGRD_Kernel(float* d_result, int width, int height, cudaTextureObject_t texObj)
{
	int row = (blockIdx.y << BLOCK_LOG_DIM) + threadIdx.y;
	int col = (blockIdx.x << BLOCK_LOG_DIM) + threadIdx.x;
	if(col < width && row < height)
	{
		int index = row * width + col;
		float v = tex1Dfetch<float>(texObj, index << 1);
		d_result[index] = (col == 0 || row == 0 || col == width -1 || row == height -1)?
				0 : saturate(5 * v);
	}
}
...
void ProgramCU::DisplayConvertGRD(CuTexImage* got, CuTexImage* out)
{
	if(out->_cuData == NULL) return;
	int width = got->GetImgWidth(), height = got->GetImgHeight();
	cudaTextureObject_t texObj = got->CreateTextureObject();
	dim3 grid((width + BLOCK_DIM - 1)/ BLOCK_DIM, (height + BLOCK_DIM - 1)/BLOCK_DIM);
	dim3 block(BLOCK_DIM, BLOCK_DIM);
	ConvertGRD_Kernel<<<grid, block>>>((float*)out->_cuData, width, height, texObj);
	got->DestroyTextureObject(texObj);
	ProgramCU::CheckErrorCUDA("DisplayConvertGRD");
}

void __global__ ConvertKEY_Kernel(float4* d_result, int width, int height,
                                  cudaTextureObject_t texKeyF4, cudaTextureObject_t texDogObj)
{
	int row = (blockIdx.y << BLOCK_LOG_DIM) + threadIdx.y;
	int col = (blockIdx.x << BLOCK_LOG_DIM) + threadIdx.x;
	if(col < width && row < height)
	{
		int index = row * width + col;
		float4 keyv = tex1Dfetch<float4>(texKeyF4, index);
		int is_key = (keyv.x == 1.0f || keyv.x == -1.0f);
		int inside = col > 0 && row > 0 && row < height -1 && col < width - 1;
		float v = inside? saturate(0.5 + 20 * tex1Dfetch<float>(texDogObj, index)) : 0.5;
...
void ProgramCU::DisplayConvertKEY(CuTexImage* key, CuTexImage* dog, CuTexImage* out)
{
	...
	cudaTextureObject_t texDogObj = dog->CreateTextureObject();
	cudaTextureObject_t texKeyF4 = key->CreateTextureObject();
	dim3 grid((width + BLOCK_DIM - 1)/ BLOCK_DIM, (height + BLOCK_DIM - 1)/BLOCK_DIM);
	dim3 block(BLOCK_DIM, BLOCK_DIM);
	ConvertKEY_Kernel<<<grid, block>>>((float4*)out->_cuData, width, height, texKeyF4, texDogObj);
	dog->DestroyTextureObject(texDogObj);
	key->DestroyTextureObject(texKeyF4);
}

void __global__ DisplayKeyPoint_Kernel(float4* d_result, int num, cudaTextureObject_t texF4Obj)
{
	int idx = IMUL(blockIdx.x, blockDim.x) + threadIdx.x;
	if(idx >= num) return;
	float4 v = tex1Dfetch<float4>(texF4Obj, idx);
...
void ProgramCU::DisplayKeyPoint(CuTexImage* ftex, CuTexImage* out)
{
	...
	dim3 block(block_width);
	cudaTextureObject_t texF4Obj = ftex->CreateTextureObject();
	DisplayKeyPoint_Kernel<<<grid, block>>>((float4*)out->_cuData, num, texF4Obj);
	ftex->DestroyTextureObject(texF4Obj);
	ProgramCU::CheckErrorCUDA("DisplayKeyPoint");
}

void __global__ DisplayKeyBox_Kernel(float4* d_result, int num, cudaTextureObject_t texF4Obj)
{
	int idx = IMUL(blockIdx.x, blockDim.x) + threadIdx.x;
	if(idx >= num) return;
	int kidx = idx / 10, vidx = idx - IMUL(kidx, 10);
	float4 v = tex1Dfetch<float4>(texF4Obj, kidx);
...
void ProgramCU::DisplayKeyBox(CuTexImage* ftex, CuTexImage* out)
{
	...
	dim3 block(block_width);
	cudaTextureObject_t texF4Obj = ftex->CreateTextureObject();
	DisplayKeyBox_Kernel<<<grid, block>>>((float4*)out->_cuData, len * 10, texF4Obj);
	ftex->DestroyTextureObject(texF4Obj);
}

///////////////////////////////////////////////////////////////////
// CuTexImage inline methods (texture object API)
///////////////////////////////////////////////////////////////////

// Create a cudaTextureObject_t for this CuTexImage backed by linear memory
inline cudaTextureObject_t CuTexImage::CreateTextureObject()
{
	cudaResourceDesc resDesc;
	memset(&resDesc, 0, sizeof(resDesc));
	cudaChannelFormatDesc chDesc;
	if(_numChannel == 1)
		chDesc = cudaCreateChannelDesc(32,0,0,0,cudaChannelFormatKindFloat);
	else if(_numChannel == 2)
		chDesc = cudaCreateChannelDesc(32,32,0,0,cudaChannelFormatKindFloat);
	else if(_numChannel == 4)
		chDesc = cudaCreateChannelDesc(32,32,32,32,cudaChannelFormatKindFloat);
	else
		chDesc = cudaCreateChannelDesc(32,0,0,0,cudaChannelFormatKindFloat);
...
inline cudaTextureObject_t CuTexImage::CreateTextureObject2D()
{
	...
	// Use linear memory with pitch for 2D texture
	cudaResourceDesc resDesc;
	memset(&resDesc, 0, sizeof(resDesc));
	cudaChannelFormatDesc chDesc;
	if(_numChannel == 1)
	...
	resDesc.res.pitch2D.devPtr = _cuData;
	resDesc.res.pitch2D.desc = chDesc;
	resDesc.res.pitch2D.width = _imgWidth;
	resDesc.res.pitch2D.height = _imgHeight;
	resDesc.res.pitch2D.pitchInBytes = _imgWidth * _numChannel * sizeof(float);
	...
	return texObj;
#else
	// Use CUDA array for 2D texture
	CopyToTexture2D();
	cudaResourceDesc resDesc;
	memset(&resDesc, 0, sizeof(resDesc));
	resDesc.resType = cudaResourceTypeArray;
	resDesc.res.array.array = _cuData2D;
...
inline void CuTexImage::DestroyTextureObject(cudaTextureObject_t obj)
{
...
void __global__ MultiplyDescriptor_Kernel(int* d_result, int num1, int num2, int3* d_temp,
                                          cudaTextureObject_t texDes1Obj, cudaTextureObject_t texDes2Obj)
{
	...
	uint4 v = tex1Dfetch<uint4>(texDes1Obj, read_idx1);
...
#pragma unroll
	for(int i = 0; i < 8; ++i)
	{
		uint4 v = tex1Dfetch<uint4>(texDes2Obj, read_idx2 + i);
...
void ProgramCU::MultiplyDescriptor(CuTexImage* des1, CuTexImage* des2, CuTexImage* texDot, CuTexImage* texCRT)
{
	...
	des2->DestroyTextureObject(texDes2Obj);
}
...
void __global__ MultiplyDescriptorG_Kernel(int* d_result, int num1, int num2, int3* d_temp,
										   ...
										   cudaTextureObject_t texDes1Obj, cudaTextureObject_t texDes2Obj,
										   cudaTextureObject_t texLoc1Obj, cudaTextureObject_t texLoc2Obj)
{
...
#error
	...
	{
		loc1[threadIdx.x] = tex1Dfetch<float>(texLoc1Obj, 2 * idx01 + threadIdx.x);
	}
	__syncthreads();
	if(idx2 >= num2) return;
	int results[MULT_BLOCK_DIMY];
	...
	int good_count = 0;
	float2 loc2 = tex1Dfetch<float2>(texLoc2Obj, idx2);
#pragma unroll
	for(int i = 0; i < MULT_BLOCK_DIMY; ++i)
	{
		if(idx1 + i < num1)
		...
		float* H, float hdistmax, float* F, float fdistmax)
{
	...
	MultiplyDescriptorG_Kernel<<<grid, block>>>((int*)texDot->_cuData, num1, num2,
		...
		texDes1Obj, texDes2Obj, texLoc1Obj, texLoc2Obj);
	loc1->DestroyTextureObject(texLoc1Obj);
	loc2->DestroyTextureObject(texLoc2Obj);
	des1->DestroyTextureObject(texDes1Obj);
	des2->DestroyTextureObject(texDes2Obj);
}
