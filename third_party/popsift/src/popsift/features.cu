...
__global__ void
compute_distance_collect( DescriptorBfMatchRow* out,
                            Descriptor*           l,
                            int                   l_len,
                            Descriptor*           r,
                            int                   r_len,
                            ...
                            int                   use_ratio_test )
{
    ...
    if( threadIdx.x == 0 )
    {
        DescriptorBfMatchRow row;
        row.best_train_idx   = match_1st_idx;
        row.second_train_idx = match_2nd_idx;
        row.best_dist_sq     = match_1st_val;
        int accept = 0;
        if( r_len < 1 )
        {
            accept = 0;
        }
        else if( ! use_ratio_test )
        {
            accept = 1;
        }
        else if( r_len < 2 )
        ...
        {
            // Lowe ratio on Euclidean distance: d1 < tau * d2  <=>  d1^2 < tau^2 * d2^2
            const float tau_sq = ratio_threshold * ratio_threshold;
            accept = ( match_1st_val < tau_sq * match_2nd_val ) ? 1 : 0;
        }
        row.accepted = accept;
        out[blockIdx.x] = row;
    }
...
std::vector<DescriptorBfMatchRow> match_descriptors_bruteforce(
    ...
    bool use_ratio_test )
{
    std::vector<DescriptorBfMatchRow> host_rows;
    if( query_count <= 0 || train_count <= 0 ) {
        return host_rows;
    }

    Descriptor* d_l = popsift::cuda::malloc_devT<Descriptor>( query_count, __FILE__, __LINE__ );
    Descriptor* d_r = popsift::cuda::malloc_devT<Descriptor>( train_count, __FILE__, __LINE__ );
    DescriptorBfMatchRow* d_out = popsift::cuda::malloc_devT<DescriptorBfMatchRow>( query_count, __FILE__, __LINE__ );

    cudaError_t cpy1 = cudaMemcpy( d_l,
                                   query_desc,
                                   static_cast<size_t>( query_count ) * sizeof( Descriptor ),
                                   cudaMemcpyHostToDevice );
    cudaError_t cpy2 = cudaMemcpy( d_r,
                                   train_desc,
                                   static_cast<size_t>( train_count ) * sizeof( Descriptor ),
                                   cudaMemcpyHostToDevice );
    if( cpy1 != cudaSuccess || cpy2 != cudaSuccess ) {
        cudaFree( d_l );
        cudaFree( d_r );
        cudaFree( d_out );
        return host_rows;
    ...
    compute_distance_collect
        <<<grid, block>>>
        ( d_out, d_l, query_count, d_r, train_count, ratio_threshold, use_ratio_test ? 1 : 0 );

    POP_SYNC_CHK;

    host_rows.resize( static_cast<size_t>( query_count ) );
    cudaError_t cpy3 = cudaMemcpy( host_rows.data(),
                                   d_out,
                                   static_cast<size_t>( query_count ) * sizeof( DescriptorBfMatchRow ),
